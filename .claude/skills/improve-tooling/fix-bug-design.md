...
- [ ] **[p3]** `cargo test <crate> --test <bin> <filter>` reports `test result: ok. 0 passed; 0 failed; 0 ignored; 0 measured; N filtered out` when `<filter>` matches zero tests — exit code 0, looks like a pass. Generic risk for any `/fix-bug` session where Phase 3 renames tests and an exit-criteria filter in the fix-section §4 still references the old name. Surfaced 2026-04-25 by Phase 2.5 finding `TPR-07-004-4-codex` against `fix-BUG-07-004.md`. Mitigations to consider: (a) update the fix-section template Phase 4 checklist to include "verify test filter matches ≥1 test before declaring exit criteria met," (b) add a `cargo-test-must-match.sh` wrapper that exits non-zero when the filter has zero matches. Low priority — Phase 2.5's adversarial review already caught it once and the gate is mandatory for platform-cfg fixes.

### Recently closed

- [x] **2026-04-25** — **BUG-07-004 close-out retrospective (no actionable improvements).** Bug fixed cleanly via the standard `/fix-bug` flow: Phase 0 → Phase -1 grounding → Phase 1 investigation → Phase 1.5 scope (point fix) → Phase 1.6 fix section → Phase 1.75 `/tp-help` (round-1 agreement, two refinements adopted) → Phase 2 finalize → Phase 2.5 Plan TPR (round-0 found 5 actionable findings, all resolved inline) → Phase 3+4 implementation + tests + commit → Phase 5 code TPR (round 0: codex clean + 1 informational meta from gemini, fixed in round 1; round 1 both clean) → Phase 5 hygiene review (clean) → Phase 5 retrospective (this entry). Defense-in-depth worked end-to-end: Phase 2.5 caught a stale exit-criteria test filter (`pty_size_is_propagated` would have matched zero tests post-rename), the parser-localization-claim mismatch, the 50×20 mode-con-wrap risk, the `let _ = child.wait()` error swallow, and the unimplemented 24×24 matrix row. None of those would have been caught by static lints — they all required adversarial human-grade reading of the plan against the implementation. Filed [p3] open item above for the generic cargo zero-match-filter silent-pass risk class. One incidental surprise (not a tooling bug): the lefthook `cargo fmt --check` runs workspace-wide regardless of staged paths, so commits to a clean file are blocked when the workspace has unrelated WIP fmt issues — user-policy choice; the user accepted "commit together" via `AskUserQuestion` rather than stash-and-pop. Pending commit (this entry).
