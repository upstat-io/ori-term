...
- [x] 2026-04-18 — **`unreviewed_plan` gate surfaces resume option when `review_pipeline` marker is present.** Root cause: /clear + /continue-roadmap had no signal to distinguish "never reviewed" from "paused mid-pipeline"; both lit up as `reviewed: false` → gate recommended "Run /review-plan now" → /review-plan restarted from Step 2, burning ~20–45 min of Step 4 /tp-help reviewer wall-clock. Observed on same section (empty-container §04) at least twice before reporting. Fix: `roadmap_scan.py` gate 1.7 now reads `review_pipeline: {stage, next_step, updated}` from section frontmatter (or `<plan_dir>/.review-pipeline-state.yaml` in whole-plan mode). When present + `next_step ∈ [2..7]`, the gate emits options `resume-review-plan (Recommended)` / `review-plan-fresh` / `pick-different` instead of the default `review-plan / proceed / pick-different` set. Resume option's description calls out the wall-clock saving. Payload gains `review_pipeline_stage` + `review_pipeline_next_step` fields for downstream consumers. Live-tested against section 04's marker: scanner returns "Resume /review-plan from Step 6" with the correct stage/next_step. Companion change on the producer side lives in `review-plan-design.md §4` (2026-04-18 entry — persistent marker). Commit: pending.
