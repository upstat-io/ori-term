...
- [x] **2026-04-19** — **Auto-fix consumer moved from `/continue-roadmap` Sonnet sub-agent to `scripts/plan-cleanup.py` (called from `/commit-push` Step 4).** Scanner output format unchanged; only the consumer changed. Payload fields (`stale_frontmatter.focus_plan_mismatches`, `stale_plan_annotations.cleanup_plan`, `bug_marker_drift.auto_fix_edits`) are now parsed by `plan-cleanup.py` (Python) instead of a sub-agent reading `workflow.md` Step 2a/2b/2c tables. `/continue-roadmap` gate severities dropped from `auto-fix` to `info` since cleanup is no longer its job. `workflow.md` is deleted. No scanner-side change. See `script-plan-cleanup-design.md` for the new consumer. Commit: `db747ab7` (consumer), no scanner commit needed.
- [x] **2026-04-17** — TPR status drift auto-fix: added `Section.tpr_mismatch` detector (scanner) + 3 rows in `workflow.md` Step 2a (sub-agent fix table). Closes the `/continue-roadmap` → `/verify-tpr` loop on sections where `third_party_review.status: findings` persists after all finding checkboxes are closed. See §4 lesson entry. _(Consumer side moved to `plan-cleanup.py` on 2026-04-19 — see entry above; detector still canonical here.)_ Commit: pending.
