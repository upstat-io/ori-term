...
- **`Production code path:`** — MANDATORY. Name the specific function, file, and line that this section modifies. From research, not guessed.
- **`Observable change:`** — MANDATORY. What will be different in the running terminal. If the answer is "nothing visible yet", the section is wrong.
- **`Context:`** — Why this section exists, grounded in research findings
- **`Reference implementations:`** — Specific patterns from reference repos found during research, with file paths
- **Subsections with `- [ ]` checkboxes** — Each task must reference a specific file and function from the research. No vague "implement X" tasks.
- **Code examples** — When a task involves a type or API change, show the CURRENT code (from research) and the target code
...
**Grounding enforcement:** If you find yourself writing a file path, type name, or function name that did NOT appear in the research findings, STOP. Go back and verify it exists. If you can't verify it, flag it explicitly in the plan as `<!-- UNVERIFIED: {claim} -->` so the review catches it.
...
5. **Must NOT replace /review-plan with inline agents.** Use the actual Skill tool to run `/review-plan`.
