...
description: Deep code review of recent changes with minimal context to avoid bias. Runs 4 sequential cold-start agents, then writes validated findings into a plan section's Third Party Review block.
...
Collect raw review inputs mechanically. Do not editorialize.
...
Run 4 review agents in sequence. Each agent:
...
Use this output format for every agent:
...
3. Apply the repo rule: do **not** reject a finding because it is "unrelated", "out of scope", or "pre-existing". Reject only findings that are factually incorrect.
...
1. Run the 4 agents sequentially, not in parallel.
2. Agents must read the full changed files, not only diffs.
3. Do not delete TPR history. Mark findings resolved with rationale.
4. Only factual incorrectness is grounds for rejection.
