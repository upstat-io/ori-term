...
description: Review code for hygiene violations and fix them. NOT refactoring — purely surface cleanliness.
...
`$ARGUMENTS` specifies the target (directory or file):
- `/code-hygiene-review src/gpu/` — review a directory
- `/code-hygiene-review src/grid/mod.rs` — review a single file
- `/code-hygiene-review src/` — review entire source
...
- Variable names scope-scaled? (no `cursor_position_result` in a 3-line scope, no `c` in a 50-line function)
...
- **Dispatch tables exempt** — Large match statements mapping escape sequences, keys, or SGR codes are not "long functions"
