#!/usr/bin/env bash
# Prepare a new release by updating the version in Cargo.toml
#
# Usage:
...
# 3. Prints next steps (review, test, commit, tag, push)
...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0              # Auto-increment pre-release"
...
update_version() {
    ...
    echo -e "${GREEN}UPDATED${NC}: Cargo.toml -> $version"
}
...
main() {
    ...
    echo -e "   ${YELLOW}git add Cargo.toml Cargo.lock && git commit -m \"chore: release v$new_version\"${NC}"
