...
- If a section shows `complete` but has many unchecked items (>5), ask the user -- the checkboxes may be stale rather than the frontmatter
...
Ask the user with options. The options depend on the blocker state:

**When there are unblocked items:**
1. **Start next task (Recommended)** -- Begin implementing the first unblocked item
2. **Show task details** -- See more context about the task (read plan, find related code)
...
The `@review` subagent (GPT-5.4) acts as a pair programmer throughout implementation. Invoke it via the Task tool at three checkpoints -- not constantly, but at the moments where a second opinion prevents wasted work.

**Checkpoint 1: Before coding (plan review)**
After reading the plan and exploring the codebase, but BEFORE writing any code:
- Use the Task tool to invoke `@review`
- Prompt: "I'm about to implement [task]. The plan says [summary]. I plan to modify these files: [list]. My approach: [brief plan]. Questions: (1) Does this approach match the plan? (2) Any edge cases or architectural concerns? (3) Am I missing any files that need changes?"
- Wait for response. If it raises concerns, address them before coding.

**Checkpoint 2: After tricky logic (mid-implementation review)**
After writing complex or non-obvious code -- skip this for straightforward changes:
- Use the Task tool to invoke `@review`
- Prompt: "I just wrote [function/module] to handle [what]. Here's the code: [paste key sections]. Plan requirement: [relevant plan]. Questions: (1) Does this match the plan? (2) Edge cases I'm missing? (3) Correctness issues?"
- Fix any issues raised before continuing.

**Checkpoint 3: After all code, before commit (final review)**
After tests pass, before updating the roadmap:
- Use the Task tool to invoke `@review`
- Prompt: "Final review before commit. Changed files: [list]. Summary: [what was done and why]. All tests pass. Review for: correctness, architecture, quality, safety, and plan alignment."
- Fix any issues raised before proceeding.

**When to skip checkpoints:**
- Checkpoint 1: Skip for trivial tasks (typo fixes, adding a single test)
- Checkpoint 2: Skip for straightforward code (no tricky logic, well-established patterns)
- Checkpoint 3: Never skip -- always do the final review
...
4. **Explore the codebase** -- Find where features should be implemented
5. **Checkpoint 1** -- Invoke `@review` with your plan (see above)
...
5. **Checkpoint 2** -- After writing tricky logic, invoke `@review` for a mid-implementation check (see above)

### After Writing Code

1. **Run checks** -- `./clippy-all.sh` and `./test-all.sh` to verify everything passes
2. **Build** -- `./build-all.sh` to verify cross-compilation
3. **Checkpoint 3** -- Invoke `@review` for final review (see above). Fix any issues before proceeding.
...
- [ ] `@review` Checkpoint 1 -- plan review (skip for trivial tasks)
- [ ] Implement feature
- [ ] Add unit tests (oriterm_core) and/or integration tests (oriterm)
- [ ] `@review` Checkpoint 2 -- mid-implementation review (skip for straightforward code)
- [ ] Run `./clippy-all.sh` -- no warnings
- [ ] Run `./test-all.sh` -- all tests pass
- [ ] Run `./build-all.sh` -- cross-compilation succeeds
- [ ] `@review` Checkpoint 3 -- final review (never skip). Fix any issues before proceeding.
- [ ] Update section file:
