...
- [ ] **Single tab**: Tab bar renders correctly with 1 tab (no animations needed)
- [ ] **Many tabs**: 20+ tabs, all narrow — clipping works, no bleed-through
- [ ] **Hover transition**: Move cursor across tabs — smooth color fades, no instant jumps
- [ ] **Tab close**: Close a middle tab — width shrinks, neighbors slide, smooth
- [ ] **Tab open**: Open new tab — width expands from zero, content fades in
- [ ] **Tab drag**: Drag a tab — shadow visible, no backing rect, smooth reorder
- [ ] **Tab drag + narrow tabs**: Drag a tab when tabs are narrow — clips hold, no bleed
- [ ] **DPI scaling**: Test at 1x, 1.5x, 2x scale — scissor rects scale correctly
- [ ] **Window resize**: Resize window while tabs visible — no clip artifacts
- [ ] **Scroll widget**: If a scroll widget is visible (future feature), content clips at scroll bounds — verify no regression from Section 01 changes
- [ ] **Rapid close**: Close 5 tabs rapidly — close animations don't pile up or corrupt state, Vecs stay in sync
- [ ] **Rapid open**: Open 5 tabs rapidly — open animations overlap correctly (each tab animates independently)
- [ ] **Close during hover**: Close a hovered tab — hover animation and close animation don't conflict
- [ ] **Tab width lock + animation**: Hover-lock active during close — multipliers respect lock priority
...
- [ ] **Animation frame budget**: During hover transition, total frame time (CPU + GPU) stays under 8ms — well within the 16.6ms budget for 60 FPS
- [ ] **Animation convergence**: All animations complete and stop requesting redraws within their declared duration + 1 frame
- [ ] **Idle overhead**: When no animations are running, zero additional work vs. current implementation
- [ ] **Scissor rect overhead**: Adding clip segments to the render pass adds negligible overhead (scissor is a hardware-native operation)

---

## 06.4 Completion Checklist

- [ ] **File size audit**: No source file (excluding `tests.rs`) exceeds 500 lines. Key files to check:
...
- [ ] All unit tests pass: `./test-all.sh`
- [ ] No clippy warnings: `./clippy-all.sh`
- [ ] Cross-compilation succeeds: `./build-all.sh`
- [ ] Tab show-through bug is fixed (20+ tabs, no bleed)
- [ ] Hover animations feel smooth and responsive
- [ ] Tab open/close animations feel natural
- [ ] Dragged tab has visible elevation (shadow)
- [ ] No visual regression at 1x, 1.5x, 2x DPI
- [ ] Animations stop requesting redraws after completion
