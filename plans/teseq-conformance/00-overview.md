...
3. **Outbound response validation** — DA/DSR/DECRQM response bytes analyzed through teseq for human-readable golden comparison
...
- [ ] CSI erase scenarios cover: ED (modes 0-3), EL (modes 0-3) with attribute preservation
...
- [ ] Report/response scenarios cover: DA1, DA2, DA3, DSR cursor position, DECRQM with outbound teseq analysis
- [ ] ESC sequence scenarios cover: DECSC/DECRC, RIS, character set designation (SCS G0/G1)
- [ ] OSC scenarios cover: title (0/1/2), CWD (7), clipboard (52), color query (4/10/11)
...
- [ ] `cargo test -p oriterm_core --test teseq` passes with zero failures
- [ ] `./test-all.sh` green — no regressions in existing test suites
 ...
 .toml sidecar ──► ScenarioSpec ─────────────────────►│ Term<RecordedListener>│
   (size, modes,                                      │    ↓                  │
    ...
    assertions)                                       │ Cursor position      │
                  ...
                  └─► pty_writes() ──► teseq ──► golden compare  [response analysis]
```
...
1. **teseq is the authoring tool, not the oracle.** Terminal state (grid, cursor, modes, events) is the authoritative golden — not teseq output. teseq's role is to make scenario files human-readable and to analyze outbound response bytes. This avoids coupling test correctness to teseq's output format, which is version-fragile and human-oriented.
...
Section 01 (TeseqHarness & Infrastructure)
  ...
  │
  └─► Section 06 (Complex Workflow Scenarios) [depends on 02-05]
...
- **Section 01 + Section 03**: RecordedEvent enum (Section 01) must capture PtyWrite payloads verbatim for Section 03's response analysis. If Section 01 only captures event types (not payloads), Section 03 cannot validate DA/DSR response content.
...
Phase 0 - Infrastructure
  ...
  Gate: `cargo test -p oriterm_core --test teseq` compiles and runs with one smoke scenario
...
Phase 4 - Verification
  ...
  Gate: Full test matrix documented, CI green, ./test-all.sh passes
```
...
| **New (teseq tests)** | **0** | **~2,500 est.** | **~2,500 est.** |
...
| 06 Complex Workflow Scenarios | ~300 | High | 01-05 |
| 07 Verification & CI | ~250 | Low | 01-06 |
| **Total new** | **~2,650** | | |
| **Total deleted** | **~0** | | |
