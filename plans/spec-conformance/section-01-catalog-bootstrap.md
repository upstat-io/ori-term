...
success_criteria:
  ...
  - "Every row's `ID` follows `{STACK}-{MNEMONIC}` format where STACK ∈ `{ECMA48, DEC, DECRECT, DECPRES, OSC, SIXEL, KG, KKBD, ITERM2, MOUSE, CHSET, HIST, AUDIO, SHINT, DFCT}` (defined in `plans/spec-conformance/00-overview.md §Catalog Row Schema`). `DECRECT` and `DECPRES` were added by Section 09A to cover `catalog/dec-rectangle-ops.md` and `catalog/dec-presentation.md` respectively. Row IDs are globally unique across all catalog files."
...
**Success Criteria:**
- [x] `plans/spec-conformance/catalog/` exists with 16 protocol-family markdown files PLUS a stub `catalog/README.md` (Section 01 owns this stub; Section 04.7 extends it with the frozen schema reference) <!-- DRIFT note: Section 09A later added 2 more catalog files (dec-rectangle-ops.md, dec-presentation.md) for 18 total. Section 01's "16" is accurate as of Section 01 close. -->
...
- [x] Create the 16 protocol-family files, each with `schema_version: "0.1-provisional"` front-matter, an H1 heading, and a 10-column markdown table header matching `plans/spec-conformance/00-overview.md:807-818` exactly (`ID`, `Spec source`, `Sequence`, `Description`, `Implementation`, `Apex layer`, `Test chain`, `Verification`, `De-facto ref`, `Notes`): <!-- DRIFT note: Section 09A later added 2 more catalog files (dec-rectangle-ops.md, dec-presentation.md) at schema_version "1.0" — they didn't go through the 0.1-provisional → 1.0 migration since 09A landed post-Section-04.7. -->
  - `catalog/ecma-48.md`
...
- [x] `plans/spec-conformance/catalog/` exists with 16 protocol-family markdown files PLUS `README.md` stub <!-- DRIFT note: Section 09A later added 2 more (dec-rectangle-ops.md, dec-presentation.md) for 18 total. -->
- [x] Every catalog file declares front-matter `schema_version: "0.1-provisional"`
