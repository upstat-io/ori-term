...
- [ ] `response_poll_roundtrip_emits_pty_write` AND `response_poll_token_requires_fulfillment` AND `response_poll_duplicate_fulfill_last_wins` green — first two in `oriterm_mux/src/pane/io_thread/response_poll/tests.rs`, the duplicate-fulfill pin in `oriterm_core`'s `host_request.rs` sibling `tests.rs`. All three test concerns live in the owning crate, never in spec_chain.
