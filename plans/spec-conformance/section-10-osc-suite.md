...
- `oriterm_core/src/term/handler/mod.rs` (**REGISTRATION SYNC**: add `Handler::set_last_command_line` override on `Term` that calls `Term::set_last_command_line(Some(line.to_owned()))`)
- `crates/oriterm_test_support/src/spec_chain/recording_handler.rs` (**REGISTRATION SYNC**: add delegate arm for `set_last_command_line` matching the existing `iterm2_file` pattern at line 317 — missing arm means spec_chain tests cannot observe OSC 633 E dispatch)
