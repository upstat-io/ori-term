...
- [ ] **Negative pins (MANDATORY per `.claude/rules/tests.md` §Negative Testing Protocol)**:
  ...
  - `osc3_non_x11_platform_no_panic` — on non-X11 platforms (macOS, Windows), feed `\x1b]3;foo;bar\x1b\\`, assert no panic and that `Term`'s OSC-3 state field matches the platform-gated expectation (Option B is preferred for 10.9 scope — the X11 property is stored in an `#[cfg(all(unix, not(target_os = "macos")))]`-gated `Term::x11_property` field, so the assertion on non-X11 platforms is that the platform-gated field is absent at compile time and no panic occurs at runtime). The `HostEffect::SetX11Property` variant does NOT exist and the negative pin MUST NOT reference it — asserting the absence of a non-existent variant would not compile; the correct pin is on the Term state field, which is absent on non-X11 platforms by `#[cfg]`.
