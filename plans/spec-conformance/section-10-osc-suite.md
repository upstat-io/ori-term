...
- [ ] **Negative pin — `ResponseToken::fulfill` is single-assignment (first-write-wins)** — `response_poll_duplicate_fulfill_rejected` — `ResponseToken::fulfill` returns `Result<(), AlreadyFulfilled>`; a duplicate fulfill is REJECTED, not a silent overwrite. Test: create a `ResponseToken<String>`, call `token.fulfill("first".into())` (succeeds), call `token.fulfill("second".into())`, assert the second call returns `Err(AlreadyFulfilled)`, then call `token.take()` and assert it returns `Some("first".into())` (the first write was preserved). **Test home**: `oriterm_core/src/effect/families/host_request/tests.rs` (the directory-module conversion landed in effect-cutover §01.2, see commit `c5a21ab5`). The first-write-wins invariant landed in effect-cutover §01.2 (`response_token_fulfill_succeeds_once` + `response_token_rejects_double_fulfillment`); this pin is the spec-conformance-side cross-reference asserting the OSC 52 / OSC 10/11/12 round-trip relies on the same single-assignment semantics. Cross-link: `plans/effect-cutover/section-01-migrate-mux-consumer.md` §01.2 blind-spot §12.
...
- [ ] `response_poll_roundtrip_emits_pty_write` AND `response_poll_token_requires_fulfillment` green — both in `oriterm_mux/src/pane/io_thread/response_poll/tests.rs` (the directory-module conversion landed in effect-cutover §01.2). `response_poll_duplicate_fulfill_rejected` green — in `oriterm_core/src/effect/families/host_request/tests.rs` (the directory-module conversion also landed in effect-cutover §01.2). All three test concerns live in the owning crate, never in spec_chain.
