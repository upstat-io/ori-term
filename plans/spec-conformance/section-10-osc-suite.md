...
- [ ] `osc99_metadata_body_form_pins_current_behavior` — Kitty's OSC 99 spec allows a two-parameter form (`OSC 99 ; metadata ; body ST` where `metadata` is `i=ID:t=TYPE:...`); the current `handle_notification_simple` at `shell_integration/interceptor.rs:120-138` drops `params[0]` (uses it only to select Osc9-vs-Osc99 source) and reads body from `params[1]`. Feed `\x1b]99;i=1:t=info;hello\x1b\\` and assert `source: Osc99, title: "", body: "hello"` — the metadata is silently dropped. This pins the deviation from Kitty's full OSC 99 spec explicitly so a future reviewer can trace the gap; promote OSC-99 row in `plans/spec-conformance/catalog/osc.md` to `verified-with-deviation` with a Notes cell citing "metadata field in two-parameter form is dropped; body-only payload is preserved".
...
- [ ] New row OSC-99 added to `plans/spec-conformance/catalog/osc.md` (status `verified-with-deviation` — metadata field in the Kitty two-parameter form is dropped; Notes cell records the deviation per `osc99_metadata_body_form_pins_current_behavior`); existing OSC-777 row promoted from `missing` to `verified` (OSC-777 already exists at `plans/spec-conformance/catalog/osc.md:57` — do NOT create a duplicate row).
- [ ] `plans/spec-conformance/catalog/shell-integration.md` SHINT-OSC-9-NOTIFY and SHINT-OSC-777-NOTIFY → `verified`.

**Validation:**

- [ ] All 9 tests pass (7 behavioral + 1 semantic pin + 1 negative pin; behavioral count grew by one for the new `osc99_metadata_body_form_pins_current_behavior` deviation pin).
