...
success_criteria:
  ...
  - "The non-image rows of `plans/spec-conformance/catalog/iterm2.md` (ITERM2-1337-REMOTEHOST, ITERM2-1337-CURRENTDIR, ITERM2-1337-COPY, ITERM2-1337-SETMARK, ITERM2-1337-REPORTCELLSIZE, ITERM2-1337-SETUSERVAR, ITERM2-1337-SHELLINTVERSION) are `verified`; `ITERM2-1337-SHELLINTVERSION` is a new catalog row added by Section 10 (it is not currently in `plans/spec-conformance/catalog/iterm2.md` — Section 10.7 adds it). `owner_section` in `plans/spec-conformance/catalog/iterm2.md` front-matter (line 5) is updated so Section 10 owns these rows and Section 14 owns ONLY `ITERM2-1337-FILE` + image-adjacent rows — cross-checked against the ownership conflict currently at `plans/spec-conformance/section-14-iterm2-images.md:55` and `plans/spec-conformance/catalog/iterm2.md:15-20` (the non-image rows)"
...
`crates/vte/src/ansi/handler.rs:270` defines `fn set_mouse_cursor_icon(&mut self, _: CursorIcon) {}` — an empty default on the Handler trait. `Term` at `oriterm_core/src/term/mod.rs` does NOT override it. `plans/spec-conformance/catalog/osc.md:29` labels OSC 22 as `stub` accordingly. Subsection **10.5** is not a test-writing exercise — it adds the Term field, the override, and the renderable surface BEFORE writing verification tests.

### F. OSC 1337 parser is monolithic; sub-op ownership is currently tangled

`crates/vte/src/ansi/dispatch/osc.rs:248-254` only routes `File=` to `Handler::iterm2_file` and drops every other sub-op to `unhandled`. `plans/spec-conformance/catalog/iterm2.md:15-20` assigns RemoteHost / CurrentDir / Copy / SetMark / ReportCellSize / SetUserVar to Section 14; `plans/spec-conformance/section-14-iterm2-images.md:55` says Section 10 already covered them. The canonical resolution (discussed with reviewers): **Section 10 owns all non-image OSC 1337 sub-ops** (the non-image rows are NOT image work). Subsection **10.7** lands the extensible sub-dispatcher and verifies non-image variants. Section 14's `owner_section` in `plans/spec-conformance/catalog/iterm2.md` front-matter is updated to `"01 (bootstrap), 10 (non-image), 14 (image)"`. The ownership conflict line at `plans/spec-conformance/section-14-iterm2-images.md:55` is updated through the flow-up edit this review authorizes (whole-plan scope would fix `section-14` directly; single-section scope notes the required update in the 10.N completion checklist — Section 14's reviewer will pick it up on its next /review-plan).
...
- `oriterm_mux/src/pane/io_thread/response_poll.rs` (remove the `#[allow(dead_code)]` gate; add the activation call in `PaneIoThread::drain_commands` — the real method at `oriterm_mux/src/pane/io_thread/mod.rs:194` — which already calls `self.poll_pending_responses()` at line 211; verify that call site exists and is reachable before removing the gate)
...
- [ ] **Response-poll activation requires EffectSink migration (GAP):** `PaneIoThread::register_host_request_response` is gated with `#[allow(dead_code)]` because the IO thread currently uses `LegacyEventSink` whose `drain_into()` is a no-op — effects are forwarded immediately as legacy `Event`s. The `response_poll.rs` module doc explicitly states: "activates when consumers migrate to `QueueingEffectSink` (in `plans/effect-cutover/`)." Section 10 CANNOT simply remove the dead-code gate without also migrating the IO thread to `QueueingEffectSink`. **The success criteria (line 14) requires live `response_poll` path activation — Option B alone does NOT satisfy this criterion.** Two valid approaches:
  ...
  - **Idle-wake hazard (depends on effect-cutover §01.2):** `poll_pending_responses()` is called ONLY from `drain_commands()` (at `oriterm_mux/src/pane/io_thread/mod.rs:211`), which is called at the top of each outer-loop iteration in `run()` (at `oriterm_mux/src/pane/io_thread/mod.rs:117`). The outer-loop iteration is blocked by `crossbeam_channel::select!` on `cmd_rx` and `byte_rx` (at `oriterm_mux/src/pane/io_thread/mod.rs:141,161`) — a fulfilled `ResponseToken` does NOT wake that `select!`. Consequence: a consumer that fulfills a token while the IO thread is idle (no new commands, no new PTY bytes) will NOT get `Effect::Pty(PtyEffect::Write {..})` until the next command or byte arrives. The fix — adding a third `select!` arm for an idle-wake channel — is scoped to `plans/effect-cutover/section-01-migrate-mux-consumer.md §01.2` ("Activate `PendingResponse` polling with idle-wake channel"). Section 10 MUST NOT add a second idle-wake mechanism; the architecturally correct home for that channel is effect-cutover §01.2. 10.2 tests that assert end-to-end token fulfillment → PTY write MUST either (a) run after effect-cutover §01.2 has landed, or (b) drive the IO thread with a synthetic `cmd_rx` send that forces a loop iteration AFTER token fulfillment (test-only). Option (b) is documented here as the interim testing path; production behavior remains dependent on effect-cutover §01.2.
...
- [ ] All five TDD matrix tests transition RED → GREEN. *(Partial — 3 of 5 landed: Term mouse-cursor-icon, injectable clock, OSC 1337 sub-dispatcher. 2 pending: `spec_chain_helper` failing-matrix pair, renderable stub regression pin.)*
...
Five of the six implementation bullets landed on 2026-04-18: the OSC 1337 sub-dispatcher refactor + 7 new `Handler::iterm2_*` methods (`crates/vte`), the `Term::mouse_cursor_icon` field + `Handler::set_mouse_cursor_icon` override + `RenderableContent` snapshot writeback (`oriterm_core`), the Option A `finish_command(now: Option<Instant>)` seam (`oriterm_core` + `oriterm_mux` caller), and the daemon-mode `PaneSnapshot::mouse_cursor_icon` transport with a project-owned `OSC22_KNOWN_ICONS` index table (`oriterm_mux::protocol::snapshot` + `oriterm_mux::server::snapshot` + `oriterm::gpu::extract::from_snapshot`). 11 TDD tests GREEN: 9 OSC 1337 dispatch tests in `crates/vte/src/ansi/dispatch/tests.rs`, 3 Term mouse-cursor-icon tests + 2 injectable-clock tests in `oriterm_core/src/term/tests.rs`. All three scripts (`./build-all.sh`, `./test-all.sh`, `./clippy-all.sh`) green.

Four items remain BLOCKED on `plans/effect-cutover/section-01-migrate-mux-consumer.md §01.1` (QueueingEffectSink migration):
1. **Response-poll activation** — `#[allow(dead_code)]` gate on `PaneIoThread::register_host_request_response` cannot be removed until the IO thread emits `Effect::HostRequest` through the EffectSink router. Validation item "grep returns zero matches" pending.
2. **`spec_chain_helper` + load-bearing OSC 133;A test** — the mux-internal helper that runs `RawInterceptor + Processor` in production order, plus the RED→GREEN pair proving the interceptor is load-bearing.
3. **`observe_renderable` completion** — the stub still returns `RungResult::pass(RungName::Renderable)` unconditionally. Completing it requires the `RenderableExpectation` extension (below).
4. **`RenderableExpectation` extension** — `scenario.rs`'s unit-struct stub needs the 7 const-constructible fields (`cells`, `hyperlink_at`, `cursor_position`, `cursor_shape`, `palette_index`, `mouse_cursor_icon`, `damaged_lines`).

The four remaining items were paused per user direction to prioritize effect-cutover §01 first (which unblocks Option A for the response-poll activation). 10.0 resumes after effect-cutover §01 lands; the remaining items are expected to complete cleanly against the baseline landed on 2026-04-18.
...
- [ ] **Negative pin — `ResponseToken::fulfill` is last-write-wins (current behavior documented)** — `response_poll_duplicate_fulfill_last_wins` — `ResponseToken::fulfill` at `oriterm_core/src/effect/families/host_request.rs:69-71` silently overwrites the prior value: `*self.slot.lock().expect("...") = Some(value);`. There is NO debug_assert guarding "fulfill-after-fulfill" — the second call replaces the first. Test: create a `ResponseToken`, call `token.fulfill("first".into())`, call `token.fulfill("second".into())`, call `token.take()`, assert the returned value is `Some("second")` (last write wins). **Test home**: `oriterm_core/src/effect/families/host_request.rs` is currently a flat file and MUST be converted to a directory module before adding tests (per `.claude/rules/test-organization.md` §Sibling `tests.rs` Pattern). Convert to `oriterm_core/src/effect/families/host_request/mod.rs` + `host_request/tests.rs`, then place the test in `tests.rs`. This test documents the current invariant so any future behavior change (e.g. adding a `debug_assert!` that panics on second fulfill, or switching to first-write-wins) is forced through this pin. If the behavior SHOULD be "first-write-wins / reject-double-fulfill" per architectural review, that is a separate bug — file via `/add-bug` with `BUG:mux:response-token-duplicate-fulfill` and update this pin's assertion accordingly when the bug fix lands. The pin's purpose is to surface the invariant, not to endorse it.
...
- [ ] `response_poll_roundtrip_emits_pty_write` AND `response_poll_token_requires_fulfillment` green — both in `oriterm_mux/src/pane/io_thread/response_poll/tests.rs` (or `io_thread/tests.rs` if `response_poll.rs` stays flat — per the Files block in the Implementation section above). `response_poll_duplicate_fulfill_last_wins` green — in `oriterm_core/src/effect/families/host_request/tests.rs` (requires converting `host_request.rs` to a directory module first per `.claude/rules/test-organization.md`). All three test concerns live in the owning crate, never in spec_chain.
...
- [ ] OSC-9 `plans/spec-conformance/catalog/osc.md` → `verified` (was `missing`). Implementation cell now cites `oriterm_mux/src/shell_integration/interceptor.rs::handle_notification_simple`.
- [ ] New row OSC-99 added to `plans/spec-conformance/catalog/osc.md` (status `verified`); existing OSC-777 row promoted from `missing` to `verified` (OSC-777 already exists at `plans/spec-conformance/catalog/osc.md:57` — do NOT create a duplicate row).
- [ ] `plans/spec-conformance/catalog/shell-integration.md` SHINT-OSC-9-NOTIFY and SHINT-OSC-777-NOTIFY → `verified`.
...
- [ ] **Negative pin** — `osc633_via_high_level_processor_drops` — run ONLY `Processor::advance(&mut term, osc633_a_bytes)` WITHOUT calling `raw_parser.advance(&mut interceptor, osc633_a_bytes)` first. Assert `term.prompt_state() == PromptState::None` (NO state change). This confirms OSC 633 dispatch is interceptor-only — the high-level Processor silently drops it. If someone accidentally adds a `b"633"` arm to `crates/vte/src/ansi/dispatch/osc.rs`, this test will fail (double-dispatch detection). NOTE: Do NOT use `SpecHarness::feed()` here — `oriterm_test_support` is NOT in `oriterm_mux`'s `[dev-dependencies]`. Use `Processor::advance` directly (same pattern as `osc9_via_processor_without_mux_drops` in 10.3).

**Catalog update:**

- [ ] OSC-133 `plans/spec-conformance/catalog/osc.md`: **Split the single `OSC-133` row into two rows before marking verified.** The A/B/C subops drive `PromptState` state machine (apex: `state-snapshot`); the D subop emits `HostEffect::CommandComplete`. **New apex layer**: `HostEffect::CommandComplete` does NOT map to any existing `ApexLayer` variant — `EffectHostNotification` is documented as "Apex: desktop notification (OSC 9/99/777)" (`crates/oriterm_test_support/src/spec_chain/scenario.rs:96-97`) and is INCORRECT for CommandComplete. Section 10.4 MUST add a new `EffectHostCommand` variant to `ApexLayer` in `crates/oriterm_test_support/src/spec_chain/scenario.rs`, matching the existing non-visual pattern: add `/// Apex: shell command lifecycle signal (OSC 133;D / CommandComplete). EffectHostCommand,` alongside the other effect-host variants, and add `ApexLayer::EffectHostCommand => Self::Effect` in the `from_apex` match arm. ALSO update `plans/spec-conformance/00-overview.md:820` to add `effect-host-command` to the canonical `ApexLayer` enum value list. New catalog rows: `OSC-133-PROMPT` (subops A/B/C, apex `state-snapshot`) and `OSC-133-CMD-COMPLETE` (subop D, apex `effect-host-command`). Implementation cells cite the interceptor + handler paths for each. The single-row `OSC-133` entry in the catalog is deleted. Also update `plans/spec-conformance/catalog/shell-integration.md::SHINT-OSC-133-PROMPT` to reference only the A/B/C rows (`state-snapshot` apex); add a new `SHINT-OSC-133-CMD-COMPLETE` cross-reference row for D (`effect-host-command` apex).
- [ ] OSC-633 `plans/spec-conformance/catalog/osc.md` → `verified` (add implementation citations).
...
- `plans/spec-conformance/section-14-iterm2-images.md:55` currently says "Section 10's OSC suite covered the non-image OSC 1337 variants". This statement is now accurate with Section 10 owning the non-image variants — but `plans/spec-conformance/catalog/iterm2.md:5` (`owner_section` front-matter field) previously assigned all variants to Section 14, and `plans/spec-conformance/catalog/iterm2.md:15-20` (the non-image row table entries) listed those rows under Section 14. This review's flow-up edit updates `plans/spec-conformance/catalog/iterm2.md:5` `owner_section` to `"01 (bootstrap), 10 (non-image), 14 (image)"` (as specified in the Catalog update block above) and promotes the non-image rows. Section 14's next /review-plan will pick up the consistent state and update `plans/spec-conformance/section-14-iterm2-images.md:55` to cite the new catalog ownership wording if needed.

**Validation:**

- [ ] All 15 tests green: `osc1337_set_mark`, `osc1337_remote_host`, `osc1337_current_dir`, `osc1337_copy`, `osc1337_report_cell_size`, `osc1337_set_user_var`, `osc1337_shell_integration_version` (7 behavioral) + `osc1337_file_still_routes_to_iterm2_file` + `osc1337_unknown_key_dropped` + `osc1337_user_vars_cap_evicts_oldest` + `osc1337_user_vars_reinsert_does_not_refresh_position` (IndexMap re-insert semantic pin) + `osc1337_ssot_cwd_direction_a` (OSC 7 → OSC 1337 overwrite) + `osc1337_ssot_cwd_direction_b` (OSC 1337 → OSC 7 overwrite) (2 CWD SSOT semantic pins) + `osc1337_copy_invalid_base64_dropped` + `osc1337_set_user_var_invalid_base64_dropped` (2 negative pins) = 15 total.
...
- [ ] Changes land in this order: `crates/vte` (new Handler trait methods + OSC 1337 sub-dispatcher) → `oriterm_core` (ALL Term fields this section adds: `mouse_cursor_icon` (10.0/10.5), `remote_host` (10.7), `user_vars` (10.7), `shell_integration_version` (10.7), `last_command_line` (10.4 — OSC 633 E sub-command), `tab_title_color` (10.9 — OSC 6), plus whatever Term fields 10.9's OSC 3/5/13/14/17/19/113/114/117/119 analysis determines are needed (e.g. `x11_property`, `mouse_fg_color`, `mouse_bg_color`, `highlight_bg_color`, `highlight_fg_color`); handler overrides land in `handler/mod.rs`, helpers in `handler/osc.rs`) → `oriterm_mux` (remove `#[allow(dead_code)]` on `register_host_request_response`; wire live call site; interceptor extensions for OSC 633 + 1337 delegated rows where applicable; mux-intercepted OSC tests added to `oriterm_mux/src/shell_integration/tests.rs` sibling unit-test module — NOT to `oriterm_mux/tests/` integration tests, which have no `pub(crate)` access to `RawInterceptor`) → `crates/oriterm_test_support` (completed renderable observer + new `RenderableExpectation` fields; registration sync for all new `Handler::iterm2_*` + new 10.9 Handler methods; NO new `mux_layer` dependency per crate-boundary constraint — see 10.0 Option A) → high-level-processor OSC tests under `oriterm_core/tests/spec_chain/osc/*` (OSC 0/1/2/4/10/11/12/8/22/50/52/104/110/111/112/1337 non-image sub-ops).
...
- [ ] **Negative pins**:
  ...
  - [ ] `ResponseToken::fulfill` is last-write-wins; duplicate-fulfill documented invariant pin (`response_poll_duplicate_fulfill_last_wins`, 10.2).
...
- [ ] **No file size violations (source files)**: per `.claude/rules/code-hygiene.md` §File Size, source files (non-`tests.rs`) stay under 500 lines. Three files Section 10 touches are already at or near the limit and will grow under the plan — each MUST be split if the Section-10 additions push over 500 lines:
  ...
  - **`oriterm_core/src/term/mod.rs`** — 468 lines today. Section 10 adds a non-trivial set of new fields (`mouse_cursor_icon`, `last_command_line`, `remote_host`, `user_vars`, `shell_integration_version`, `tab_title_color` plus 10.9 OSC state fields `x11_property`, `mouse_fg_color`, `mouse_bg_color`, `highlight_bg_color`, `highlight_fg_color`). Count projected new lines BEFORE landing; if over 500, extract the OSC-state bundle into a dedicated submodule `oriterm_core/src/term/osc_state/` (mod.rs with a struct + accessors, embedded as a single field on `Term<S>`) per `.claude/rules/code-hygiene.md` §File Size. This preserves SSOT (Term still owns the bundle) while keeping the file under 500 lines.
  - **`oriterm_core/src/term/handler/mod.rs`** — 438 lines today. Section 10 adds `iterm2_set_mark`, `iterm2_remote_host`, `iterm2_current_dir`, `iterm2_copy`, `iterm2_report_cell_size`, `iterm2_set_user_var`, `iterm2_shell_integration_version`, `set_mouse_cursor_icon`, `set_x11_property`, `set_mouse_fg_color`, `set_mouse_bg_color`, `set_highlight_bg_color`, `set_highlight_fg_color` (13 new Handler overrides). Count projected new lines BEFORE landing; if over 500, move the OSC overrides into `handler/osc.rs` (which already exists for OSC helpers) and leave `handler/mod.rs` as the `impl Handler for Term<S>` shell that delegates to helper methods defined in `handler/osc.rs`. Preserves SSOT (trait impl still lives with Term) without the file-size violation.
...
- [ ] `plans/spec-conformance/00-overview.md` Quick Reference + mission success criterion **"Verification chain complete per row"** incremented (checkboxes for promoted rows); the **"Effect/State separation enforced"** criterion gets a note that 10.2 activated the dormant response-poll arm.
...
- [ ] **SIZE_VIOLATION (308 top-level items)** — documented exception in the Structural Note at the top of this section. Section 10 is correctly subsectioned into 10.0–10.9 + 10.R + 10.N; the 20-item heuristic is a "subsection or split" prompt and this section has taken the subsection path. Do not re-litigate without a concrete TPR-anchor-preservation plan. **This exception applies to Section 10 only**; future sections do NOT inherit this license.
- [ ] **BLOAT_RISK for plan docs** — `plans/spec-conformance/00-overview.md` (951), `plans/spec-conformance/index.md` (574), `plans/spec-conformance/section-08-ecma-48-baseline.md` (701), `plans/spec-conformance/section-10-osc-suite.md` (1391). These are plan-doc files with cross-section reference weight; splitting is out of scope for Section 10 (each would require its own dedicated /review-plan cycle). Flagged for future /review-plan sessions that own those files; Section 10 does not modify 00-overview.md, index.md, or section-08 beyond the cross-references called out in 10.N.
