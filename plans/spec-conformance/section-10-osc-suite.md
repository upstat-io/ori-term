...
third_party_review:
  ...
  updated: "2026-04-17"
  rounds_completed: 25
  notes: "user-accepted via accept-with-findings at iter_cap_reached — 1 finding (TPR-10-109 SIZE_VIOLATION documented exception) filed as - [ ] item in §10.R"
sections:
  - id: "10.0"
...
- `oriterm_mux/src/pane/io_thread/response_poll.rs` (remove the `#[allow(dead_code)]` gate on `register_host_request_response`; `PaneIoThread::drain_commands` at `oriterm_mux/src/pane/io_thread/mod.rs:194` ALREADY calls `self.poll_pending_responses()` at `oriterm_mux/src/pane/io_thread/mod.rs:211`, so no poll-call wiring is required — the task is strictly removing the dead-code gate and wiring the live `register_host_request_response` call site from the OSC 52 handler path)
...
- [ ] `./build-all.sh` + `./test-all.sh` + `./clippy-all.sh` green after 10.1's changes (per CLAUDE.md "run these after every change"; section-level `/tpr-review` + `/impl-hygiene-review` are gated at 10.N's Final Verification).
...
- [ ] **IO-thread regression scan** — because 10.2 flips response polling from dormant to live and removes the `#[allow(dead_code)]` gate on a method routed through `PaneIoThread`, a whole-workspace run is not sufficient. Explicitly re-run the three IO-thread-adjacent test modules that could regress from the live activation: `timeout 150 cargo test -p oriterm_mux pane::io_thread::tests`, `timeout 150 cargo test -p oriterm_mux server::snapshot`, `timeout 150 cargo test -p oriterm_mux pane::shutdown` (if the latter module does not exist by that exact path, substitute the pane-shutdown test module path reported by `cargo test -p oriterm_mux -- --list`). Any failure in these modules is a regression introduced by 10.2 and blocks the subsection from marking complete, independent of the whole-workspace `./test-all.sh` result.
- [ ] `oriterm_core/tests/teseq/osc.rs::osc_clipboard` regression test unchanged and green.
- [ ] `./build-all.sh` + `./test-all.sh` + `./clippy-all.sh` green after 10.2's changes (per CLAUDE.md "run these after every change"; section-level `/tpr-review` + `/impl-hygiene-review` are gated at 10.N's Final Verification).
...
- [ ] `./build-all.sh` + `./test-all.sh` + `./clippy-all.sh` green after 10.3's changes (per CLAUDE.md "run these after every change").
...
- [ ] `./build-all.sh` + `./test-all.sh` + `./clippy-all.sh` green after 10.4's changes (per CLAUDE.md "run these after every change").
...
- [ ] `[TPR-10-109-codex][medium]` `plans/spec-conformance/section-10-osc-suite.md:1434` — SIZE_VIOLATION (308 top-level items) remains as a documented exception per the Structural Note's anchor-preservation rationale; reviewer suggested moving audit-history (10.R) and closeout detail (10.N) into subordinate files to meet the 20-item cap.
  ...
  Required plan update: Deferred to a targeted `/review-plan` follow-up that evaluates a concrete split proposal (split 10.N only vs split 10.R too vs accept exception permanently). This finding is FILED as an OPEN `- [ ]` in 10.R per `/tpr-review §7` — downstream `/review-bugs` / `/fix-next-bug` workflows own it.
...
- [ ] **No file size violations (source files)**: per `.claude/rules/code-hygiene.md` §File Size, source files (non-`tests.rs`) stay under 500 lines. Three files Section 10 touches are already at or near the limit and will grow under the plan — each MUST be split if the Section-10 additions push over 500 lines:
  ...
  - **`oriterm_core/src/term/handler/mod.rs`** — 442 lines today (post §10.0 partial landing). Section 10 adds `iterm2_set_mark`, `iterm2_remote_host`, `iterm2_current_dir`, `iterm2_copy`, `iterm2_report_cell_size`, `iterm2_set_user_var`, `iterm2_shell_integration_version`, `set_mouse_cursor_icon` (these already landed as trait/override pairs), `set_x11_property`, `set_mouse_fg_color`, `set_mouse_bg_color`, `set_highlight_bg_color`, `set_highlight_fg_color` (13 new Handler overrides total; 8 landed in §10.0, 5 pending in §10.9). Count projected new lines BEFORE landing; if over 500, move the OSC overrides into `handler/osc.rs` (which already exists for OSC helpers) and leave `handler/mod.rs` as the `impl Handler for Term<S>` shell that delegates to helper methods defined in `handler/osc.rs`. Preserves SSOT (trait impl still lives with Term) without the file-size violation.
