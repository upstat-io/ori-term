...
- [ ] **Negative pins**:
  ...
  - [ ] OSC 3 on non-X11 platform does not panic and emits no `HostEffect::SetX11Property` (`osc3_non_x11_platform_no_panic`, 10.9).
...
- [ ] **No file size violations (test files)**: the 500-line limit applies to `tests.rs` sibling files too per `.claude/rules/code-hygiene.md` §File Size. Four test files Section 10 touches are already multiples of the limit today; Section 10 MUST NOT grow them further without splitting. Each MUST either be split BEFORE Section 10's additions land, OR the new tests for this section MUST be placed in a newly-created submodule sibling `tests.rs` (never appended to the existing oversized file). Plan this BEFORE writing the new tests:
  - **`oriterm_core/src/term/tests.rs`** — 2612 lines today (5.2× limit). Section 10.0's `term_set_mouse_cursor_icon_stores_icon` test lands here per the TDD bullet. Split BEFORE adding: extract existing tests into submodules by subject — `term/tests/cursor.rs`, `term/tests/mode.rs`, `term/tests/palette.rs`, `term/tests/shell_state.rs`, etc. — via a `term/tests/mod.rs` aggregator. Then add 10.0's new test to the appropriate submodule (likely `term/tests/cursor.rs` for `mouse_cursor_icon`). NO new tests appended to `term/tests.rs` until the split lands.
  - **`oriterm_mux/src/pane/io_thread/tests.rs`** — 2339 lines today (4.7× limit). Section 10.2's `response_poll_emits_pty_write_on_fulfill`, `response_poll_roundtrip_emits_pty_write`, `response_poll_token_requires_fulfillment` tests land here (if `response_poll.rs` stays flat). Split BEFORE adding: extract existing tests into `io_thread/tests/drain_commands.rs`, `io_thread/tests/handle_bytes.rs`, `io_thread/tests/snapshot_flip.rs`, etc., via an aggregator. Add 10.2's response-poll tests to a new `io_thread/tests/response_poll.rs` submodule. Prefer the `response_poll/mod.rs + response_poll/tests.rs` directory-module conversion documented in 10.2 Files block — that placement avoids adding to the oversized `io_thread/tests.rs` entirely and is the preferred path.
  - **`oriterm_mux/src/shell_integration/tests.rs`** — 984 lines today (1.97× limit). Section 10 heavily extends this with the `spec_chain_helper` + OSC 7/9/99/133/633/777 mux tests across subsections 10.0, 10.3, 10.4, 10.8. Split BEFORE adding: extract existing tests into `shell_integration/tests/` submodules by OSC family — `tests/osc7.rs`, `tests/osc133.rs`, `tests/osc9_99_777.rs`, `tests/osc633.rs` — via a `shell_integration/tests/mod.rs` aggregator that ALSO re-exports the `spec_chain_helper` test-only module so downstream test files can call it. The `spec_chain_helper` itself lives at `shell_integration/tests/spec_chain_helper.rs` and is `pub(super) use`d from the aggregator. All new Section 10 mux tests go into the appropriate per-OSC submodule, not the monolithic `tests.rs`.
  - **`oriterm_core/src/term/handler/tests/osc.rs`** — 1131 lines today (2.26× limit). Section 10 cites this as a regression guard but does NOT add new tests; nonetheless the existing file is a standing Broken Window per the policy at CLAUDE.md. Split BEFORE Section 10 closes: extract into `handler/tests/osc/` submodule tree — `osc/title.rs` (OSC 0/1/2), `osc/color.rs` (OSC 4/10/11/12/104/110/111/112), `osc/hyperlink.rs` (OSC 8), `osc/clipboard.rs` (OSC 52), `osc/cursor.rs` (OSC 22/50), `osc/iterm2.rs` (OSC 1337), via an `osc/mod.rs` aggregator. Same-commit rename; citations in the catalog `Test chain` cells updated to the new paths.
...
- [ ] **BLOAT_RISK for source / test files Section 10 touches** — handled above in the "No file size violations (source files)" and "No file size violations (test files)" items. Each cited file has an explicit split plan or a cap-before-extending rule.
