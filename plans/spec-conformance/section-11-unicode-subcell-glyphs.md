...
success_criteria:
  ...
  - "Every row in `catalog/unicode-subcell.md` is `verified`: USC-BLOCKS (U+2580–U+259F, half blocks + quadrants), USC-BOX (U+2500–U+257F), USC-BRAILLE (U+2800–U+28FF), USC-LEGACY-SEXTANT (U+1FB00–U+1FB3B), **USC-LEGACY-OCTANT (U+1CD00–U+1CDE5 — NEW)**."
...
inspired_by:
  ...
  - "Unicode 16 chart PDFs — definitive shape reference for every glyph (archival copy at `plans/spec-conformance/specs/unicode-symbols-legacy.pdf`)."
depends_on: ["05", "08", "09A"]
third_party_review:
  status: none
  updated: null
review_pipeline:
...
**Depends on:**
- Section 05 (deterministic GPU env for goldens).
...
**File(s):** `oriterm/src/gpu/builtin_glyphs/legacy_computing/octants.rs` (new), `oriterm/src/gpu/builtin_glyphs/legacy_computing/mod.rs` (extended dispatch), `oriterm/src/gpu/builtin_glyphs/mod.rs` (top-level dispatch — add octants to `is_builtin_glyph` / `render_builtin_glyph`), `oriterm/src/font/mod.rs` (font-shaper skip logic — add octants to the "skip font shaping for these codepoints" set alongside blocks / braille / sextants), sibling `tests.rs`.

- [ ] Create `oriterm/src/gpu/builtin_glyphs/legacy_computing/octants.rs` driven by the canonical bitmask table artifact produced in §11.0 — NOT by extrapolating the sextant arithmetic in `legacy_computing/mod.rs`. The Canvas is a 2×4 grid (2 columns × 4 rows) per cell. Each U+1CD00..=U+1CDE5 codepoint maps to an 8-bit bitmask describing which of the 8 sub-cells are filled, with bit order row-major 0..7 per the canonical artifact.
- [ ] Canonical-mapping guard test: a sibling `tests.rs` test asserts the codepoint→mask table in `octants.rs` is byte-identical to the canonical artifact from §11.0 (parse the artifact, compare). Any divergence is a compile-time regression.
- [ ] Braille-vs-octant rendering-model check: before sharing ANY Canvas helper across `braille.rs` and `octants.rs`, a sibling test asserts that the two renderers treat their 2×4 grids differently — braille uses Unicode-dot-order bit numbering (`oriterm/src/gpu/builtin_glyphs/braille.rs`, dots 1-8 per UCD) and octants use row-major 0..7. If the test is not added, the implementations must NOT share a Canvas renderer (per `.claude/rules/impl-hygiene.md §Algorithmic DRY` — shared code requires shared semantics).
...
- [ ] Add octants to the top-level built-in-glyph dispatch in `oriterm/src/gpu/builtin_glyphs/mod.rs` — `is_builtin_glyph(ch)` and `render_builtin_glyph(ch, canvas)` must both route U+1CD00..=U+1CDE5 to the legacy_computing module. Without this wiring the legacy_computing dispatch is unreachable from the render pipeline.
- [ ] Add octants to the font-shaper skip set in `oriterm/src/font/mod.rs` alongside the existing U+1FB00..=U+1FB9F block. The shaper MUST NOT attempt to shape octant codepoints via the system font — the built-in renderer is canonical. Without this addition the built-in renderer is silently bypassed when the configured font advertises coverage of an octant codepoint.
- [ ] Sibling tests: render a few representative octant codepoints via the built-in renderer, verify the bitmask matches the canonical artifact from §11.0 (not a separate re-encoding of the chart).
- [ ] **Validation**: octant codepoints render correct shapes per chart PDF; dispatch routes through the built-in renderer for every U+1CD00..=U+1CDE5 codepoint; font shaping is never invoked for those codepoints.

---

## 11.2 Spec_chain visual tests for every subcell glyph family

**File(s):** `oriterm/src/gpu/visual_regression/spec_chain/glyphs/subcell.rs` (new — visual rungs 5–8 live in the `oriterm` crate per Section 05's crate-boundary rule; `oriterm_core` does not own GPU-path tests), goldens in `oriterm/src/gpu/visual_regression/references/spec_chain/glyphs/` (or the path `VisualSpecHarness` resolves to — follow the convention established in `oriterm/src/gpu/visual_regression/spec_chain/visual_harness.rs`).
...
- [ ] **Sparse golden pins (one per family):** for each glyph family (half blocks, quadrants, sextants, octants, braille), pick one representative codepoint that exercises the family's bitmask (e.g., quadrant `▟` U+259F = 0x0F mask, octant with all 8 bits set, etc.) and commit an exact golden PNG at the canonical 12pt @ 96 DPI cell metrics.
- [ ] **Render path:** the test invokes `render_frame_cached()` via `VisualSpecHarness` — the production render path — per `.claude/rules/tests.md §GPU cached render testing` and `.claude/rules/oriterm.md §Visual regression`. Never use a simplified test-only render path; it would mask production bugs.
...
- [ ] Update catalog rows in `catalog/unicode-subcell.md` to `verified`: USC-BLOCKS, USC-BRAILLE, USC-LEGACY-SEXTANT, USC-LEGACY-OCTANT (the existing `USC-BOX` row is covered by earlier sections).
- [ ] **Validation**: every family's sparse golden test passes with 0-pixel diff; the exhaustive raster sweep passes for every codepoint in scope; the font-precedence test passes; the braille/octant adjacency test passes; back-to-back runs produce 0-pixel diff.
...
- [ ] §11.0 audit populated + octant block/range drift normalized across `plans/spec-conformance/` + canonical octant bitmask mapping artifact committed (see §11.0 completion criteria).
- [ ] Failing test matrix written FIRST.
- [ ] **Matrix dimensions**: glyph family (half / quad / sext / oct / braille) × {sparse-golden, exhaustive-raster, font-precedence, adjacency} × apex (visual golden for golden-pin / semantic-bitmask for raster / visual for font-precedence + adjacency).
- [ ] **Semantic pins**: octant canonical-mapping guard test + braille-vs-octant rendering-model check + font-precedence test + braille/octant adjacency test are each a distinct regression guard; removing any one of them must cause a test failure in the crate it lives in.
...
- [ ] Octants wired into `oriterm/src/gpu/builtin_glyphs/mod.rs` top-level dispatch AND `oriterm/src/font/mod.rs` shaper skip set — built-in renderer is unconditionally selected for U+1CD00..=U+1CDE5.
- [ ] Every subcell glyph family has: (a) a sparse golden pin, (b) an exhaustive semantic raster sweep over every codepoint, (c) a font-precedence test, (d) a braille/octant adjacency test covers both 2×4 families.
...
- [ ] Catalog rows in `catalog/unicode-subcell.md` all `verified`: USC-BLOCKS, USC-BRAILLE, USC-LEGACY-SEXTANT, USC-LEGACY-OCTANT.
- [ ] All existing visual_regression glyph tests pass without modification.
...
**Exit Criteria:** Every Unicode subcell glyph catalog row is `verified`; octants implemented and exhaustively tested (sparse goldens + per-codepoint semantic raster + font-precedence + braille/octant adjacency); octant block/range citations are normalized across every plan-file; the built-in renderer is canonical and the font shaper never handles U+1CD00..=U+1CDE5.
