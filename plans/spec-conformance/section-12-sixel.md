...
- `oriterm_core/src/image/sixel/mod.rs` — `SixelParser` coupled state machine (436 lines): byte intake at `mod.rs:33-73,115-190`, operator dispatch at `mod.rs:260-268`, palette rebuild at `mod.rs:87-91`, repeat clamp at `mod.rs:336` (`count.min(MAX_DIMENSION)` inside `emit_sixel`; `MAX_DIMENSION` const at `mod.rs:17`), raster attrs Pan/Pad ignored at `mod.rs:311-330` (`apply_raster_attrs` reads only `params[2]`/`params[3]`), `SetToBg` enum at `mod.rs:22-30` and undrawn-pixel fill at `mod.rs:222-227` (currently same opaque black as `DeviceDefault`).
