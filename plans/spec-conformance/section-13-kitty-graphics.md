...
- [ ] `[HYG-13.1-005][NOTE]` `oriterm_core/tests/spec_chain/kitty/transmissions.rs:40-43,67-69` + `fixtures.rs:68-74` — temp-dir cleanup uses manual `fs::remove_file` + `fs::remove_dir` tails that leak on assertion failure (the cleanup runs only if the assertion-block completes without panic). Per `.claude/rules/code-hygiene.md` §Temporal Coupling & RAII Guards, paired setup/cleanup operations must be enforced structurally via `Drop`. Both codex and gemini independently confirmed; fix path corrected by codex: `tempfile = "3"` is already at `crates/oriterm_test_support/Cargo.toml:12`, AND `oriterm_core` already dev-depends on `oriterm_test_support`, so the cleanest fix is to add a `TempDirGuard` RAII helper to `crates/oriterm_test_support/src/spec_chain/` rather than adding a new `tempfile` dep to `oriterm_core`. **Implementation anchor:** add `pub struct TempDirGuard(tempfile::TempDir)` to `oriterm_test_support/src/spec_chain/` with `pub fn new(prefix: &str) -> TempDirGuard` and `pub fn path(&self) -> &Path` accessors; `Drop` handles cleanup on all exit paths including assert-failure panic. Migrate `kitty/fixtures.rs::tmp_dir` to return `TempDirGuard`; migrate both `kitty_transmission_file_reads_from_disk_without_removing_source` and `kitty_transmission_tempfile_reads_and_removes_source` to hold the guard for the test lifetime and drop the manual cleanup tails. TDD: add a regression test that panics inside a `TempDirGuard` scope and verifies the dir is cleaned up on the unwinding path. Target resolution: **§13.1 close-out retroactive** (do as a follow-up commit within the same session-of-work arc) OR §13.2 entry (chunked transmission will need temp files too).
