...
- [ ] **Implement the animation timer** — wire `Term::advance_animations` into the pane IO thread. Add the missing scheduler API first, then wire the path:
  ...
  - In `oriterm/src/app/event_loop_helpers/mod.rs`, feed the deadline into the new `WindowRoot::scheduler_mut().request_frame_at(deadline)` so the `RenderScheduler` wakes up the event loop at the right time.
