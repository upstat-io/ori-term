...
- [x] Write failing test matrix BEFORE implementation (TDD per `.claude/rules/tests.md` §TDD for Bugs). Matrix authored in `oriterm_core/tests/spec_chain/kitty/actions.rs`; first run is the green run (post-§13.0.5 code already satisfies the per-action / per-format / per-transmission invariants).
...
- [x] **Negative pin** — `kitty_action_unknown_a_value_does_not_route_to_transmit` feeds `a=x,i=31` and asserts `placement_count != 0` — fails if `parse.rs:199` is narrowed to `Err(...)` OR routed to `Transmit` (which would yield 0 placements). Paired with the semantic pin per `.claude/rules/tests.md` §Matrix Clamping.
...
- [ ] `[HYG-13.1-002][LEAK:algorithmic-duplication][medium]` **PtyEffect::Write filter skeleton duplicated across 12+ spec_chain modules** — `oriterm_core/tests/spec_chain/kitty/fixtures.rs:75-110` (§13.1 added `reply_bytes`, `reply_contains`, `count_replies_exact`) joins a pre-existing pattern also present in `dec_presentation/{decrqss,decrqde,decrsps,decrqpsr,decrqupss}.rs`, `private_modes/{mode_2026,mode_2031,decnkm_decbkm,matrix}.rs`, `dec_rect_ops/decrqcra.rs`, `osc/iterm2_non_image.rs`, `osc/missing_rows.rs`, and `pilots/`. Each independently re-implements `effects.iter().filter_map(|e| matches!(e, Effect::Pty(PtyEffect::Write { bytes, .. })) ...)` with minor closure variations. Both codex and gemini independently confirmed the duplication during the §13.1 impl-hygiene phase 4 cross-check. Per `.claude/rules/impl-hygiene.md` §Algorithmic DRY, 3+ instances (let alone 12+) mandates extraction. **Implementation anchor:** new module `crates/oriterm_test_support/src/spec_chain/effect_filters.rs` exporting `pty_writes(&SpecHarness) -> impl Iterator<Item = (&[u8], PtyWriteKind)>`, `pty_writes_of_kind(&SpecHarness, PtyWriteKind) -> ...`, `pty_write_concat(&SpecHarness, PtyWriteKind) -> Vec<u8>`, `count_exact_pty_writes(&SpecHarness, &[u8], PtyWriteKind) -> usize`. Migrate all 12+ call sites to consume these helpers; drop the per-file duplicates. §13.1's `fixtures.rs` helpers (`reply_bytes`, `reply_contains`, `count_replies_exact`) become thin wrappers OR are removed in favor of direct consumption. TDD: extract the helpers with unit tests in `oriterm_test_support/src/spec_chain/effect_filters/tests.rs`, then mechanically migrate call sites one module at a time with per-migration `cargo test` green gate. **Owning subsection:** this is a cross-cutting spec_chain test-infrastructure refactor that touches files across §12, §13, §09A-§09D, §10, and §11 — NOT §13's scope alone. Fix by opening a new plan `plans/spec-conformance-infra/effect-filter-extraction.md` via `/create-plan`, or by adding a cross-cutting subsection to the existing `spec-conformance` plan at §00. Target resolution: before §13.R close-out of the full §13 section (which gives multiple subsections to amortize the extraction cost).

- [ ] `[HYG-13.1-003][NOTE]` `oriterm_core/tests/spec_chain/kitty/actions.rs::action_format_transmission_matrix_completeness` — matrix completeness test asserts only cell count (84), not per-cell dispatch identity. Closing this clamp requires a new `DispatchArgs` variant in `crates/oriterm_test_support/src/spec_chain/recording_handler.rs` that captures the parsed `KittyCommand` (currently the kitty APC path collapses into the generic `Other { method: "apc_dispatch" }` arm at `recording_handler.rs:328-331`, which cannot distinguish per-action routing). Gemini's phase-4 confirmation of the original fix was unverified confabulation — codex surfaced the `dispatched_calls` API gap by reading the recording handler. **Implementation anchor:** add `DispatchArgs::KittyApc { command: Box<KittyCommand> }` variant to `recording_handler.rs`; override `apc_dispatch` in `RecordingHandler` to parse the payload and record the typed variant when the first byte is `G`. Then the matrix test iterates `dispatched_calls` and asserts one `KittyApc` entry per cell with the expected `cmd.action` / `cmd.format` / `cmd.transmission`. TDD: add a unit test in `recording_handler/tests.rs` that feeds a kitty APC and asserts the typed variant appears. Target resolution: **§13.3 TPR gate** (covers §13.0 through §13.3; the matrix clamping becomes load-bearing for §13.3's animation dispatch once the harness can tell apart `a=f` from `a=a` at the dispatch rung).
...
- [ ] `[HYG-13.1-006][LEAK:algorithmic-duplication][medium]` **Manual temp-dir cleanup-on-drop pattern duplicated across 5+ call sites outside §13 scope** — `oriterm/src/scheme/loader/tests.rs:176-198,211-218`, `oriterm/src/scheme/tests.rs:219-269` (manual `std::env::temp_dir().join(...)` + `remove_dir_all` / `remove_file` tails), `oriterm_ipc/tests/ipc_roundtrip.rs:31` (same pattern for IPC socket test dir), `oriterm_core/tests/teseq/harness/reseq.rs:30` (teseq harness tmp_out). Each independently re-implements the broken "manual cleanup that leaks on assertion-failure panic" pattern that HYG-13.1-005 just fixed for the §13.1 kitty transmissions tests. Per `.claude/rules/impl-hygiene.md` §Algorithmic DRY, 3+ instances (let alone 5+) mandates extraction via the existing canonical home. **Implementation anchor:** migrate all 5+ call sites to consume `oriterm_test_support::spec_chain::TempDirGuard` (already exists). For sites outside `spec_chain` (scheme, IPC, teseq), either (a) promote `TempDirGuard` from `spec_chain/` to a crate-root `oriterm_test_support::TempDirGuard` re-export (cleanest — it is already protocol-neutral), or (b) keep it scoped and add a sibling `oriterm_test_support::test_tmp_dir::TempDirGuard` for non-spec_chain consumers. TDD: existing `temp_dir_guard/tests.rs` regression matrix already covers the invariant; each migration only needs to assert tests still pass after replacing the manual pattern. **Owning subsection:** cross-cutting test-infrastructure refactor touching `oriterm/`, `oriterm_ipc/`, and `oriterm_core/tests/teseq/` — NOT §13's scope. Fix by opening a new plan `plans/spec-conformance-infra/temp-dir-raii-migration.md` via `/create-plan`, or by adding a cross-cutting subsection to the existing `spec-conformance` plan at §00 (same pattern as HYG-13.1-002's effect-filter extraction). Target resolution: before the workspace's next infrastructure-hygiene sweep; the existing `TempDirGuard` in-tree is ready to consume today.
