...
| Image cache has NO resize/reflow handler                                                                  | `oriterm_core/src/image/cache/mod.rs` has `prune_scrollback` and `remove_placements_in_region` but nothing for grid resize. Placements with out-of-bounds columns may persist after resize. | Section 07   | Complete    |
...
| 09 | DEC Private Modes (full)                               | `section-09-dec-private-modes.md`                    | In Progress |
