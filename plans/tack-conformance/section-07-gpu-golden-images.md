...
re_review_reason: "Section 04 (post-Agent-1 expansion) defines the `LiveSession::finish(self) -> ExitStatus` cleanup contract that Section 07 MUST consume. The original Section 07 draft just dropped `LiveSession`, which loses the exit-status assertion (the M5 Codex finding fix). Section 07 also references `tack_framework::scenarios::{color, graphic_rendition, character_sets, modes}::*` consts that Sections 05/06 currently define inline in test targets — those sections are also blocked on re-review. Section 07 MUST be re-reviewed after Sections 04/05/06 land to: (a) call `live.finish()` after `render_to_pixels`, (b) use `outcome.golden_name()` from `LiveSession`'s sibling `ScenarioOutcome` instead of hand-passed strings, (c) confirm the const paths match where Sections 05/06 actually put them. ADDITIONAL DRIFT introduced by Section 05's Agent-1 / Agent-2 / Agent-3 review pass: (a) `PhaseSpec` + `ScenarioRunner::run_phase` is a NEW framework primitive for capturing mid-flow tack content — Section 07 currently has NO `run_phase_with_session_at` GPU bridge, so phase-captured screens cannot have GPU goldens. Per the open architectural decision in Section 05.5b, the default verdict is that Section 07 stays with stable-screen `LiveSession` only and the modes-family GPU golden uses `TACK_MODES_AM` (capturing the always-visible `(os)` cap). If Agent 4 / user overrides this verdict, Section 05 must add `run_phase_with_session_at` and Section 07 must add per-cap modes goldens. (b) `tack_version_supported()` is now AND-combined into `ScenarioRunner::available()` — Section 07 inherits this gate automatically, including the loud-skip diagnostic. (c) Section 05 introduces snapshot directory paths under `oriterm_core/tests/tack/test_menu/snapshots/` (insta uses module-path-based namespacing); Section 07's PNG goldens at `oriterm/tests/references/tack_*.png` are a separate tree and do NOT collide, but the `outcome.snapshot_name()` / `outcome.golden_name()` SSOT helpers from Section 04 must be the only naming source — Section 07 MUST NOT rebuild the format string. (d) Per Pivot 2 of Agent 3, Section 07's `depends_on` is refined: Section 07 only needs Section 04 + Section 05's cap_coverage CONTRACT (specifically the per-section `CapCoverageContribution` pattern) — NOT Section 05's full body. Section 07 has zero cap-coverage contribution of its own; the goldens are pixel regression tests, not cap exercisers. Section 07 can therefore start as soon as Section 04 lands; the 05/06 dependencies are about const path locations only."
