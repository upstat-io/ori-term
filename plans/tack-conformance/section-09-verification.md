...
success_criteria:
  ...
  - "All Section 01-08 deliverables verified: shared PtySession, terminfo provisioning, scenario framework, 50+ catalog scenarios (18 test_menu + ~12 tools_menu active + ~23 direct-VTE cap xcheck from Section 06 Track B), 6 GPU goldens, full kf1-kf63 + cursor + editing + 62 modified-key keyboard cross-check (12 test functions, ~150+ assertions)"
...
- [ ] Compile the test matrix table into this section file:
  ...
  | keyboard terminfo_xcheck | kf1-kf63 + cursor (app+normal) + editing + 62 modified keys + normal-mode Home/End | n/a | ✓ | ? | infocmp tests: tools-skip; pure encoder tests (normal_mode_emit_csi): ✓ |
