...
- [ ] Test matrix table populated: every scenario from Sections 05-08 listed with its sizes and current pass/fail/skip status, with drift-gate tests (`cap_coverage_matrix`, `begin_testing_inventory`, `tools_menu_inventory`, `status_reports_inventory`) as explicit first-class rows
- [ ] DA/DSR cross-validation: vttest menu6 coarse markers and tack status_reports structural validators agree (or divergences documented as bugs)
...
- [ ] Cross-compile for `x86_64-pc-windows-gnu` succeeds (workspace + test targets + all-targets clippy)
- [ ] No regressions in performance tests (alloc_regression including `rss_stability_under_sustained_output`, event_loop_helpers tests)
- [ ] Flake-proofing: full tack suite passes 5 runs x 2 thread-modes with zero failures
- [ ] Worktree cleanliness: no `.snap.new` or modified `.png` artifacts after all test runs
- [ ] Pure encoder keyboard tests run (not skip) on Windows; `cap_coverage_matrix` runs unconditionally on all platforms
- [ ] All mission criteria in `00-overview.md` checked — including the two currently unchecked: "All tests skip cleanly when tack/tic unavailable" and "./test-all.sh green, ./build-all.sh green, ./clippy-all.sh green — no regressions"
...
- [ ] Run vttest text tests (post-Section-01 dedup): `timeout 150 cargo test -p oriterm_core --test vttest`. Expected: 29 test functions producing 198 snapshots, all pass, zero `.snap.new` files.
- [ ] Run vttest GPU goldens: `timeout 150 cargo test -p oriterm --features gpu-tests -- vttest_golden`. Expected: 11 test functions producing 98 PNG goldens, all match.
...
- [ ] **Flake-proofing gate (full tack suite).** Run the tack integration tests and tack_cap_xcheck 5 consecutive times each at both `--test-threads=1` and `--test-threads=4` (20 total invocations). This is the plan's final determinism gate — Sections 07 and 08 each proved their own subsets; this proves the full suite:
  ```
...
- [ ] **Worktree cleanliness check.** After running all test suites above, verify `git status` produces no untracked `.snap.new` files, no modified `.png` goldens, and no other test artifacts:
  ```
...
- [ ] If vttest menu6 fails to see its coarse markers while tack's validators pass (or vice versa):
...
- [ ] Document the comparison in this section file as a "Cross-validation result: ✓ (vttest coarse + tack precise agree)" or "Cross-validation result: divergence found, fixed by [commit]"
...
- [ ] Run alloc regression tests: `timeout 150 cargo test -p oriterm_core --test alloc_regression`. Expected: all 5 non-ignored tests pass. The key tests covering CLAUDE.md performance invariants:
...
- [ ] Run event_loop_helpers tests: `timeout 150 cargo test -p oriterm event_loop_helpers`. Expected: pass — the `compute_control_flow()` invariants are unchanged (zero idle CPU beyond cursor blink).
...
- [ ] Test matrix populated for all suites (vttest text, vttest GPU, tack text, tack GPU, keyboard terminfo_xcheck, terminfo unit tests) with inventory/meta tests as explicit rows
- [ ] Drift-gate tests verified as first-class matrix entries: `cap_coverage_matrix` (platform-universal), `begin_testing_inventory`, `tools_menu_inventory`, `status_reports_inventory`
- [ ] All test suites pass on Linux
- [ ] Cross-validation between tack DA/DSR scenarios and vttest menu6 — vttest coarse markers and tack structural validators agree (or documented divergence + fix)
- [ ] Performance regression tests (alloc_regression — including `rss_stability_under_sustained_output`, event_loop_helpers) all green
...
- [ ] Flake-proofing gate: full tack suite + tack_cap_xcheck pass 5 runs x 2 thread-modes (20 total invocations) with zero failures
- [ ] Worktree cleanliness: `git status --porcelain -- '*.snap.new' '*.png'` produces no output after all test runs
- [ ] `./build-all.sh` green
...
- [ ] Lint all targets: `cargo clippy --workspace --all-targets` AND `cargo clippy --workspace --target x86_64-pc-windows-gnu --all-targets` — covers test/bench targets that `clippy-all.sh` misses
- [ ] Cross-platform skip matrix documented in this file
- [ ] **Mission criterion: "All tests skip cleanly when tack/tic unavailable"** — verified via 09.4 skip matrix + Windows cross-compile + pure encoder tests confirmed to RUN (not skip) on Windows
- [ ] **Mission criterion: "./test-all.sh green, ./build-all.sh green, ./clippy-all.sh green — no regressions"** — verified via 09.1 + 09.3 + 09.4
- [ ] Pure encoder keyboard tests (`cursor_keys_normal_mode_emit_csi`, `editing_keys_normal_mode_emit_csi`) verified to RUN on Windows (not skip)
- [ ] `cap_coverage_matrix` verified to run unconditionally on all platforms (no tool gate)
- [ ] All `00-overview.md` Mission Success Criteria checkboxes ticked (including the two currently unchecked)
