...
- [x] **Run** `timeout 150 cargo test -p oriterm_core --test tack -- test_menu::cap_coverage_matrix::tack_cap_coverage_matrix --exact`. The test must pass on the FIRST run. If it fails, EITHER add the missing caps to a section's `covered` OR justify them in a section's `exempt` — never silence the test. (Per TPR-05-026: the fully qualified path is required because the integration target uses `mod test_menu::cap_coverage_matrix` containing `fn tack_cap_coverage_matrix`; bare-name filters and shorter `--exact` prefixes match zero tests against this target layout.)
...
- [x] **Run the cap-coverage matrix test as a SEPARATE gate** so a failure isn't masked by another scenario panicking first:
  ...
  ```
  Must pass on every commit. The negative pin on stale exemptions makes this a tight feedback loop for Sections 06/08 cleanup work — if either section adds caps to its own `CONTRIBUTION.covered` without removing the matching `CONTRIBUTION.exempt` entries (or another section's `exempt`), this test fires. **Per TPR-05-026** (Codex final TPR iter 1): the fully qualified test path is required because cargo's `--exact` matches against the FULL test path including the module hierarchy; bare-name filters like `tack_cap_coverage_matrix` or shorter prefixes like `test_menu::cap_coverage_matrix --exact` match ZERO tests against this target's layout.
...
- [x] `[TPR-05-026][medium]` `plans/tack-conformance/section-05-test-menu-scenarios.md:2202`, `plans/tack-conformance/section-05-test-menu-scenarios.md:2206` — the recorded standalone cap-coverage verification command reports green while executing zero tests.
  ...
  Verified by re-running `cargo test -p oriterm_core --test tack -- test_menu::cap_coverage_matrix::tack_cap_coverage_matrix --exact` — `1 passed; 0 failed` against the live target layout. The shorter forms `cargo test -p oriterm_core --test tack tack_cap_coverage_matrix` and `cargo test -p oriterm_core --test tack -- test_menu::cap_coverage_matrix --exact` both report `0 passed; 0 failed; 0 ignored` (the false-green failure mode this fix corrects).
