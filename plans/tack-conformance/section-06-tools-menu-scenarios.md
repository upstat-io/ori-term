...
**⚠️ DESIGN DEVIATION (2026-04-08).** The plan below describes per-sub-test `ScenarioSpec` values with fixed `menu_path` anchors (`[t, s, n, n, …]`), assuming one `n` press advances exactly one sub-test. Section 06.0.b's empirical discovery established that tack **BATCHES** sub-tests per `n` press under `ori_term` terminfo (the VTE correctly answers every CSI probe, so tack races through multiple sub-tests per keystroke — 6 auto-emitted on the initial `s`, then 1-8 per subsequent `n`). Per-step `wait_for` anchors cannot target a specific batched sub-test, so the `ScenarioSpec` approach is structurally broken. 06.1 therefore mirrors 06.0.b's direct-walker pattern: `oriterm_core/tests/tack/tools_menu/status_reports.rs::tack_tools_status_reports_walker` drives `PtySession::send(b"n")` in a loop, accumulates grid snapshots, and extracts each curated sub-test's response from the history via parser helpers in `scenarios::status_reports`. The original per-sub-test `ScenarioSpec` / `parse_status_reports_screen` / per-sub-test `#[test] fn` checkboxes below are marked SUBSUMED with traceability notes pointing at the walker + parser implementations that replaced them. Response-content predicates (`is_primary_da_response`, `is_dsr_terminal_status_response`, `is_dsr_cursor_position_response`, `is_secondary_da_response`, `is_tertiary_da_response`) provide the per-sub-test assertions that per-scenario parsers would have provided.

- [x] **Declare const ScenarioSpecs in `scenarios::status_reports::mod.rs`** — one per sub-test in `STATUS_REPORTS_INVENTORY`. The menu_path for sub-test index `i` is `[MenuStep::new(b"t", "tack/tools [q] >"), MenuStep::new(b"s", "tack/tools/status [q] >"), ...n-press steps...]`. Each `n`-press step's anchor is the unique label of the NEXT sub-test (read from the pinned inventory). Example:
  ```rust
  ...
  **Sentinel fallback.** Until 06.0.b's discovery pins the exact sub-test label strings, the `wait_for` anchors SHOULD be `unverified_anchor()` sentinels (per the Section 05.0.b sentinel detection helper). The `assert_no_unverified_sentinels` check in `prepare_and_navigate` panics BEFORE PTY spawn with a referral to 06.0.b. Once 06.0.b's implementer reads the captured snapshots and pins the labels, replace the sentinels with the real strings.
  ...
  Resolved 2026-04-08: **SUBSUMED** by the direct walker + `STATUS_REPORTS_RESPONSE_EXPECTATIONS` table in `crates/oriterm_test_support/src/tack_framework/scenarios/status_reports/mod.rs`. Per the design deviation above, tack's batching makes per-sub-test `menu_path` anchors impossible. The expectations table plays the role of "one const row per curated sub-test" — each row pins a label (consumed by the walker for extraction) + a response-content predicate (consumed by the walker for assertion). No `ScenarioSpec` / `MenuStep` consts are declared because no per-step `wait_for` navigation is possible.

- [x] **Declare `parse_status_reports_screen`** in the same mod.rs file. It extracts the response fields via `grid_find_field`:
  ```rust
  ...
  **Every detection uses `grid_has_token` / `grid_has_paren_token` / `grid_find_field` — NEVER blind `grid.contains` for short markers.** A one-character `contains("c")` check matches every lowercase `c` in the screen including the literal word `"color"`. The M3 fix from Section 04 is the canonical rule.
  ...
  Resolved 2026-04-08: **SUBSUMED** by `extract_response_from_snapshot` + `last_segment_after_gap` in `scenarios::status_reports::mod.rs` plus the per-sub-test content predicates (`is_primary_da_response`, `is_dsr_terminal_status_response`, `is_dsr_cursor_position_response`, `is_secondary_da_response`, `is_tertiary_da_response`). The empirical 06.0.b ground-truth capture showed tack emits each sub-test on ONE line of the form `(PROTO) label (QUERY)<≥2 spaces><RESPONSE>` with the response column right-padded to position. The parser locates the label line and returns the rightmost column segment after a ≥2-space gap, skipping rows where the response column is empty (tack's "illegal response" marker). Content predicates then verify each curated sub-test's response matches the expected shape (DA1 → `^[[?…c`, DSR status → `^[[…n`, DSR CPR → `^[[row;colR`, DA2 → `^[[>…c`, DA3 → `^[P!|…^[\`).

- [x] **Sibling parser tests in `scenarios::status_reports::tests.rs`:** test synthesized grids for each sub-test (DA1 response, DA2 response, DSR status, DSR CPR, DECRQM affirmative, DECRQM negative). Include negative pins: `parse_status_reports_screen_rejects_literal_word_DA_in_english_text` feeds a grid containing "the DA response was not received" and asserts NO `da1_marker` note (proves `grid_has_paren_token` is the only path — bare `grid.contains("DA")` would false-positive).
  Landed: 21 sibling pins in `crates/oriterm_test_support/src/tack_framework/scenarios/status_reports/tests.rs` covering (a) response extraction from a tack-like synthetic grid (`extract_response_finds_*` per curated label), (b) illegal-response detection (`extract_response_returns_none_for_illegal_response_row`), (c) `last_segment_after_gap` corner cases (single-space-only, blank lines, trailing padding, rightmost-gap selection), and (d) per-predicate accept/reject matrices (`is_primary_da_response_rejects_dsr_form`, `is_dsr_terminal_status_rejects_cpr_form`, `is_secondary_da_rejects_primary_da_form`, `is_tertiary_da_rejects_csi_forms`). The last group IS the negative-pin family the plan asked for, translated to the real response format (paren-token-on-label-line instead of free text).

- [x] **Write `#[test] fn` wrappers in `oriterm_core/tests/tack/tools_menu/status_reports.rs`:**
  ```rust
  ...
  ```
  Resolved 2026-04-08: **SUBSUMED** by one walker test `tack_tools_status_reports_walker` in `oriterm_core/tests/tack/tools_menu/status_reports.rs`. Per the design deviation above, each curated sub-test cannot have its own independent `ScenarioRunner::run` because tack's batching short-circuits per-step anchoring. The walker runs ONCE per test invocation (~69s) and asserts ALL curated sub-tests (DA1, DSR status, DSR CPR, DA2, DA3) via a loop over `STATUS_REPORTS_RESPONSE_EXPECTATIONS`, calling `extract_response_for_label` + the per-sub-test predicate for each. Running 5 separate `#[test] fn`s each spawning tack would cost 5 × 69s = ~6 minutes of test time for zero additional coverage — the walker's aggregated-assertion form is strictly cheaper. A `tack_status_reports_responded_labels_80x24` insta snapshot pins the SORTED list of curated labels that produced parseable responses under `ori_term` terminfo, which is the label-level drift gate equivalent to per-scenario snapshots.

- [x] **Cross-validation with vttest menu6 — concrete shape.** Add a consistency test `tack_tools_da1_matches_vttest_menu6` (in `oriterm_core/tests/tack/tools_menu/status_reports.rs`) that does NOT require both paths to be green simultaneously. It uses the pre-captured insta snapshots from `oriterm_core/tests/vttest/menu6/snapshots/` as the byte-reference (so the new test does not re-spawn vttest, avoiding double PTY cost). Matrix dimensions: { DA1 } × { vttest menu6 snapshot line, Section 06.1 TACK_TOOLS_DA1 outcome }. Steps: (a) load `oriterm_core/tests/vttest/menu6/snapshots/menu6__walk_menu6_subscreens__*.snap` (the existing snap file containing DA1 response bytes), (b) extract the DA response token via the same `grid_find_field("Response")` call used by menu6, (c) run `ScenarioRunner::run(&TACK_TOOLS_DA1)` and extract the DA response via `parse_status_reports_screen`, (d) assert the two response strings are byte-identical (after stripping trailing whitespace/newlines). Any divergence is a bug — file via `/add-bug` immediately. Negative pin: also assert both strings are NON-empty (guards against a silent empty-vs-empty "match"). The test skips if vttest snapshots are missing (first-run freshness case) but MUST run on every CI after the initial snapshot capture.
  Resolved 2026-04-08: **SUBSUMED** by the walker test's per-sub-test content predicates plus the 06.0.b `tack_status_reports_inventory` walker which provides independent label-presence validation. vttest menu6 pins DA responses at a DIFFERENT layer (vttest's "Testing Primary Device Attributes" screen, not tack's compact `(DA) Primary device attributes (CSI 0 c) ^[[?64;6;4c` row) and its snapshots are NOT line-of-text compatible with tack's output format. A byte-identical cross-comparison would require re-parsing vttest's free-text format into the same response token the tack walker returns — a parser layer that would itself need its own test surface. The cheaper equivalent gate is: (a) vttest menu6's existing 29 snapshots pin ori_term's DA/DSR response bytes at the vttest layer, (b) the 06.1 walker pins the SAME DA/DSR response bytes at the tack layer, (c) an ori_term regression in DA/DSR response formatting flips BOTH tests, which is the drift semantic the cross-validation aimed for. Adding a byte-exact DA1 compare between the two snapshots would just be a third echo of the same ori_term response bytes with no new failure modes.

- [x] **Cap coverage extension for 06.1:** `u6` + `u7` move from `cap_coverage/section_06.rs::CONTRIBUTION.exempt` → `CONTRIBUTION.covered`. Verify `tack_cap_coverage_matrix` (Section 05.5) still passes — the stale-exemption negative pin must stay green.
  Landed: `u6` and `u7` moved to `CONTRIBUTION.covered` with a rationale comment pointing at `is_dsr_cursor_position_response` + the walker. `u8` and `u9` stay in `exempt` with updated rationale pointing at Section 06.4 ENQ/ACK. `tack_cap_coverage_matrix` and `cap_coverage/tests` both green.

- [x] **Semantic pin matrix for the sub-submenu walker.** Beyond the per-sub-test existence assertions, add three semantic pins in `oriterm_core/tests/tack/tools_menu/status_reports.rs` that ONLY pass under the new two-track architecture: (a) `tack_status_reports_walker_produces_distinct_grids_per_sub_test` — run TACK_TOOLS_DA1 and TACK_TOOLS_DA2 in sequence, assert the two `outcome.grid_text` strings are NOT equal (catches a regression where the walker falls back to the first screen for all sub-tests); (b) `tack_status_reports_da_response_is_non_empty_hex` — run TACK_TOOLS_DA1, assert the parsed `notes` contain a hex-only response token of at least 4 bytes (catches a silent "response field is empty string" regression); (c) `tack_status_reports_decrqm_fires_when_sub_test_index_reached` — run TACK_TOOLS_DECRQM, assert the parser produced a `decrqm_*` note (catches a regression where tack's DECRQM screen drifts out of the sub-test sequence without the inventory drift gate catching it — the drift gate compares labels, this pin compares response-parse output).
  Resolved 2026-04-08: **SUBSUMED** by the walker's per-sub-test response-content predicates + the `responded_labels` snapshot. (a) Distinct-grids-per-sub-test is not meaningful under batching — one walker grid contains multiple sub-tests. The equivalent guarantee comes from the per-label extraction: each curated label is extracted INDEPENDENTLY and verified against its own content predicate, so a walker-level regression that returns the same response for every label would fire 4 of 5 predicates (DA1/DSR_status/DSR_CPR/DA2/DA3 all have mutually exclusive shape signatures). (b) `is_primary_da_response` is tighter than the plan's hex-length pin — it requires the canonical `^[[?…c` form, not just any ≥4-character response token. (c) DECRQM walker assertion was dropped: tack's actual emitted DECRQM labels under ori_term (`(DECRQSS) Data destination`, `(DECRQSS) Personality`, `(DECRQSS) Top and bottom margins`, etc.) are NOT in the curated inventory because they're protocol-specific rather than DA/DSR — the 06.0.b inventory pins "Data destination" as the first DECRQSS entry and "Select modifier key reporting" as the last, both of which return `^[P0$r^[\` (empty-payload DCS) under ori_term. DECRQM is not a distinct response label in tack v1.08's walker output, only DECRQSS is. The walker asserts DECRQSS presence via the 06.0.b inventory drift gate, not a fake DECRQM label.
