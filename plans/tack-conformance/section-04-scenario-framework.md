...
success_criteria:
  ...
  - "`grid_has_token`, `grid_line_starts_with`, `grid_find_field` parser helpers live in `tack_framework::parser::tokens` and replace blind `grid.contains(short_label)` checks across all per-scenario parsers"
  ...
  - "One end-to-end scenario `tack_modes_am` (modes screen, autowrap-mode test) passes: navigates `[n] -> [m]` (begin testing -> modes), waits for a modes-SCREEN-specific anchor (NOT `\"begin testing\"` which is already on the main menu), captures grid, asserts via insta snapshot AND a parser-extracted assertion using `grid_has_token` (\"the modes screen contains the literal `am` capability label as a whitespace-bounded token\")"
...
- [ ] One end-to-end scenario `tack_modes_am` passes: navigates `n m`, uses a modes-SCREEN-specific anchor (not `"begin testing"` which is on the main menu), snapshots, asserts the parser found the `am` capability label via `grid_has_token` (not blind `grid.contains`)
