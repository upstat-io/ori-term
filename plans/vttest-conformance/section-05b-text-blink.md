...
- [x] Add `text_blink_opacity: f32` field to `FrameInput` at `frame_input/mod.rs` (after `fg_dim`). Default: `1.0` (no blink effect). Doc comment: "Opacity multiplier for cells with `CellFlags::BLINK` (0.0 = hidden, 1.0 = visible)."
- [x] Update `FrameInput::test_grid()` to set `text_blink_opacity: 1.0`
- [x] Update all existing `FrameInput` construction sites that use struct literal syntax (search for `fg_dim:` to find them — the new field must be added alongside)
...
- [x] In `fill_frame_incremental()` at `prepare/dirty_skip/mod.rs`: same pattern — per-cell dim based on BLINK flag.
- [x] In `fill_frame()` at `prepare/unshaped.rs`: same pattern.
- [x] For builtin geometric glyphs emitted directly via `push_glyph()` at `prepare/mod.rs:421`: use `cell_dim` instead of `fg_dim`.
- [x] Verify decorations at `prepare/decorations.rs` — underlines/strikethrough on BLINK cells should also fade. Currently decorations use a fixed `1.0` alpha. If the decoration is on a BLINK cell, the decoration should fade too. This may require passing `text_blink_opacity` to the decoration emitter.
- [x] `./build-all.sh` and `./clippy-all.sh` green after changes
...
- [x] Add `text_blink: CursorBlink` field to `App` at `app/mod.rs` (below `cursor_blink`)
- [x] Initialize in constructor at `constructors.rs` with configurable interval (default: 500ms)
- [x] Add config fields to `TerminalConfig`:
...
- [x] Add settings dialog toggle for text blink fade (in terminal section, below cursor blink fade)
- [x] In `event_loop.rs` `about_to_wait()`: call `text_blink.update()` and mark dirty if changed. Always run text blink (unlike cursor blink which is gated by `blinking_active` — text blink is always active because any cell in any pane could have BLINK)
- [x] Update `ControlFlowInput`: add `text_blink_active: bool` and `next_text_blink_change: Instant`. Scheduling logic: use `min(next_blink_change, next_text_blink_change)` when both are active.
...
- [x] In `redraw/mod.rs`: compute `text_blink_opacity` from `self.text_blink.intensity()` (apply fade toggle). Set `frame.text_blink_opacity = text_blink_opacity;` before calling `prepare()`.
- [x] In `redraw/multi_pane/mod.rs`: same — compute opacity, set on each pane's `FrameInput`.
- [x] Config reload handler at `config_reload/mod.rs`: update `text_blink.set_interval()` when `text_blink_rate_ms` changes.
- [x] Text blink timer does NOT reset on keypress (unlike cursor blink). It runs continuously.
...
- [x] GPU visual regression test: `text_blink_visible` — render a cell with `CellFlags::BLINK` and `text_blink_opacity = 1.0`, verify glyph is visible (pixel matches non-blink cell)
- [x] GPU visual regression test: `text_blink_hidden` — render with `text_blink_opacity = 0.0`, verify BLINK cell's glyph is invisible (pixel matches background) while non-BLINK cells remain visible
- [x] GPU visual regression test: `text_blink_half` — render with `text_blink_opacity = 0.5`, verify BLINK cell's glyph brightness is intermediate
- [x] Prepare unit test: `blink_cell_gets_dimmed_fg` — construct a `FrameInput` with one BLINK cell and `text_blink_opacity = 0.5`, run through `prepare_frame`, verify the glyph instance has alpha = `fg_dim * 0.5`
- [x] Prepare unit test: `non_blink_cell_ignores_text_blink_opacity` — same input but cell without BLINK flag, verify alpha = `fg_dim`
- [x] Config test: `text_blink_defaults` — verify `text_blink_rate_ms = 500` and `text_blink_fade = true`
- [x] Config test: `text_blink_from_toml` — verify deserialization
- [x] Existing `compute_control_flow` tests still pass
- [x] `./test-all.sh` green
...
- [x] Cells with `CellFlags::BLINK` visually blink (not static)
- [x] Blink rate defaults to 500ms (configurable via `text_blink_rate_ms`)
- [x] Smooth fade configurable via `text_blink_fade` (settings dialog toggle)
- [x] Non-BLINK cells unaffected by text blink opacity
- [x] vttest menu 2 screens 13-14 show blinking text
- [x] GPU visual regression tests pass (3 opacity levels)
