...
- [ ] `oriterm_mux/src/pty/lifecycle.rs`: Define `pub(crate) trait PtyLifecycle: Send { fn kill(&mut self) -> io::Result<()>; fn wait(&mut self) -> io::Result<ExitStatus>; fn try_wait(&mut self) -> io::Result<Option<ExitStatus>>; fn process_id(&self) -> Option<u32>; }`. No tests for the trait definition itself — exhaustiveness is enforced by the impls.
- [ ] `oriterm_mux/src/pty/mod.rs`: Add `pub(crate) mod lifecycle;` and re-export `PtyLifecycle`.
- [ ] `oriterm_mux/src/pty/spawn.rs`: Implement `PtyLifecycle for PtyHandle` by delegating to the existing `child` field.
- [ ] `oriterm_mux/src/pane/mod.rs`: Change `PaneParts.pty` field from `PtyHandle` to `Box<dyn PtyLifecycle + Send>`. Update every `Pane` method that touches `self.pty` (`.kill()`, `.wait()`, `.process_id()`).
...
- [ ] **Build verification**: `./build-all.sh && ./test-all.sh` (debug) + `cargo test --release -p oriterm_mux` (release). Phase 1A must be green BEFORE Phase 1B begins.

**Phase 1B — Mux adopt path (oriterm_mux, library crate, no Windows deps):**
- [ ] `oriterm_mux/src/pty/adopt/tests.rs` (write FIRST, expect compile errors): tests for `AdoptedPtyHandle::new`, `take_reader/writer/signal` (Some-then-None matrix), `process_id` round-trip, `PtyLifecycle::kill()` returns `Ok(())`, `PtyLifecycle::wait()` blocks until signaled by an EOF helper.
- [ ] `oriterm_mux/src/pty/adopt/mod.rs`: Define `AdoptedSignal` (raw signal handle wrapper, `#[cfg(windows)]` body, no-op stub on other platforms) with `Drop` that closes owned handles. Define `AdoptedPtyHandle` wrapping `Option<Box<dyn io::Read + Send>>`, `Option<Box<dyn io::Write + Send>>`, `Option<AdoptedSignal>`, `client_pid: Option<u32>`, plus a `wait_event: Arc<(Mutex<Option<ExitStatus>>, Condvar)>` so `PtyLifecycle::wait()` blocks until the reader thread signals EOF. Implement `take_reader`/`take_writer`/`take_signal`/`process_id` and `PtyLifecycle for AdoptedPtyHandle`. `kill()` returns `Ok(())` (ori_term did not spawn the process; the console host owns child lifecycle). No Windows deps in the public API — accepts trait objects.
- [ ] `oriterm_mux/src/pty/mod.rs`: Add `pub(crate) mod adopt;` and re-export `AdoptedPtyHandle`, `AdoptedSignal`.
- [ ] `oriterm_mux/src/domain/handoff/tests.rs` (write FIRST): tests for `adopt_pane()` that pass a `std::io::Cursor` reader, an in-memory writer (`Vec<u8>` behind a `Mutex`), and a stub `AdoptedSignal`. Verify the returned `Pane` has a non-zero `PaneId`, the IO thread has started (snapshot version > 0 after a tick), and dropping the pane shuts the IO thread down cleanly within 1 second.
- [ ] `oriterm_mux/src/domain/handoff/mod.rs`: Implement `pub fn adopt_pane(pane_id: PaneId, domain_id: DomainId, reader: Box<dyn io::Read + Send>, writer: Box<dyn io::Write + Send>, signal: AdoptedSignal, client_pid: Option<u32>, rows: u16, cols: u16, scrollback: usize, theme: Theme, mux_tx: &mpsc::Sender<MuxEvent>, wakeup: &Arc<dyn Fn() + Send + Sync>) -> io::Result<Pane>`. Body mirrors `LocalDomain::spawn_pane()` steps 2-8 (skip step 1 — no `spawn_pty()`): construct `AdoptedPtyHandle`, take handles, create shared atomics, create `Term` + `IoThreadEventProxy`, spawn writer thread, spawn IO thread, spawn reader thread, assemble `PaneParts` with the `AdoptedPtyHandle` boxed as `Box<dyn PtyLifecycle + Send>`.
- [ ] `oriterm_mux/src/domain/mod.rs`: Add `pub(crate) mod handoff;` and re-export `adopt_pane`. Note: `domain/local.rs` becomes `domain/local/mod.rs` + `domain/local/tests.rs` only if it does not already follow the directory pattern (it currently does not — see test-organization rule; if local.rs has no tests of its own, the file form is fine). The new `handoff/` directory module is mandatory because it has tests.
- [ ] **Build verification**: `./build-all.sh && ./test-all.sh` (debug) + `cargo test --release -p oriterm_mux` (release). Phase 1B must be green and the adopt path test must pass on Linux/macOS/Windows BEFORE Phase 2 begins.

**Phase 2 — Registry helpers (oriterm, `#[cfg(windows)]`):**
- [ ] `oriterm/src/platform/default_terminal/registry/tests.rs` (write FIRST): tests covering the matrix `{ register → is_registered, unregister → !is_registered, register → re-register (idempotent), unregister-without-register → no error, is_registered with corrupted GUID → false }`. Each test uses a scoped registry subtree (`HKCU\Software\Classes\oriterm_test_<random>`) and cleans up in `Drop`. All tests are `#[cfg(windows)]`; on non-Windows the file compiles to an empty module.
- [ ] `oriterm/src/platform/default_terminal/registry/mod.rs`: Define `pub(crate) const ORITERM_TERMINAL_CLSID: windows_sys::core::GUID` (generate once via `uuidgen`, hardcode as a 128-bit literal). Implement `pub(crate) fn register_all(exe_path: &Path) -> io::Result<()>` — writes `DelegationTerminal` (ori_term's CLSID) and `DelegationConsole` (`{2EACA947-7F5F-4CF2-97EA-C9E8AED6FC68}`, the Windows Terminal OpenConsole.exe CLSID) to `HKCU\Console\%%Startup` as REG_SZ, and creates `HKCU\Software\Classes\CLSID\{oriterm-GUID}\LocalServer32` with `exe_path`. Uses `windows-sys` `RegSetValueExW`/`RegCreateKeyExW`.
- [ ] `oriterm/src/platform/default_terminal/registry/mod.rs`: Implement `pub(crate) fn unregister_all() -> io::Result<()>` — deletes both `%%Startup` values and the `CLSID\{oriterm-GUID}` key tree. Uses `RegDeleteValueW`/`RegDeleteTreeW`. Idempotent — missing keys are not errors.
- [ ] `oriterm/src/platform/default_terminal/registry/mod.rs`: Implement `pub(crate) fn is_registered() -> bool` — checks if `DelegationTerminal` under `%%Startup` matches ori_term's CLSID.
- [ ] **Build verification**: `./build-all.sh && ./test-all.sh` (debug) + `cargo test --release -p oriterm` (release). Registry tests run on Windows only via `#[cfg(windows)]`; on Linux/macOS the file is an empty stub. Phase 2 must be green BEFORE Phase 3 begins.

**Phase 3 — COM server (oriterm, `#[cfg(windows)]`):**
- [ ] `oriterm/src/platform/default_terminal/handoff/tests.rs` (write FIRST): cross-platform compile-time tests — `assert_send::<HandoffData>()`, `assert_sync::<HandoffData>()` (must be `Send + Sync` to cross thread boundaries from RPC thread to main thread). Plus a parser test that constructs a synthetic `TERMINAL_STARTUP_INFO`-shaped struct and verifies `HandoffData::from_startup_info()` correctly copies title, icon_path, dimensions, and handles empty/null `BSTR` fields gracefully (empty title → `String::new()`, null icon → `None`).
- [ ] `oriterm/src/platform/default_terminal/handoff/mod.rs`: Define `HandoffData` struct (reader: `Box<dyn io::Read + Send>`, writer: `Box<dyn io::Write + Send>`, signal: `AdoptedSignal`, client_pid: `u32`, title: `String`, icon_path: `Option<String>`, initial_rows: `u16`, initial_cols: `u16`). Implement `from_startup_info(info: &TERMINAL_STARTUP_INFO) -> (String, Option<String>, u16, u16)` as a pure helper that the COM callback uses. Implement `ITerminalHandoff3` COM interface via `windows-implement` — in `EstablishPtyHandoff`: create pipe pair via `CreatePipe`, return write-end/read-end to caller via the `[out]` params, duplicate `[in]` handles (`signal`, `reference`, `server`, `client`) via `DuplicateHandle` into a new `AdoptedSignal`, copy `TERMINAL_STARTUP_INFO` BSTR fields to owned `String`s via `from_startup_info()`, construct `HandoffData`, send through `mpsc::Sender`.
- [ ] `oriterm/src/platform/default_terminal/handoff/mod.rs`: Implement `IDefaultTerminalMarker` (empty marker interface, UUID `746E6BC0-AB05-4E38-AB14-71E86763141F`) on the same COM object — conhost checks for this to verify default terminal support.
- [ ] `oriterm/src/platform/default_terminal/tests.rs` (write FIRST): test that `IClassFactory::CreateInstance` constructs a `HandoffServer` bound to the provided sender and returns `S_OK`. Test `run_com_server` happy path with a mock COM environment (or `#[ignore]` + manual verification if mocking COM is impractical — document the manual repro steps).
- [ ] `oriterm/src/platform/default_terminal/mod.rs`: Implement `pub(crate) fn run_com_server() -> io::Result<HandoffData>` following the 9-step lifecycle sequence above. Internally creates `let (tx, rx) = mpsc::channel::<HandoffData>()`, passes `tx` to the `IClassFactory` constructor, registers the factory with COM, then blocks on `rx.recv()`. Returns the received `HandoffData` on success. Contains `CoInitializeEx`, `CoRegisterClassObject`, wait-for-handoff loop, `CoRevokeClassObject` on error. Implement `IClassFactory` (COM class factory) that creates `HandoffServer` instances bound to the channel sender.
- [ ] **Build verification**: `./build-all.sh && ./test-all.sh` (debug) + `cargo test --release -p oriterm` (release). The Send/Sync compile checks run on all platforms; the COM activation tests are Windows-only. Phase 3 must be green BEFORE Phase 4 begins.

**Phase 4 — CLI and main thread integration (oriterm):**
- [ ] `oriterm/src/cli/tests.rs` (write FIRST or extend existing): test that parsing `--register-default` produces a `Command::RegisterDefault` variant; same for `--unregister-default`. On non-Windows, the command parses but the dispatcher returns "not supported" without panicking.
- [ ] `oriterm/src/cli/mod.rs`: Add `--register-default` and `--unregister-default` subcommands. `--register-default` calls `registry::register_all()` with the current exe path. `--unregister-default` calls `registry::unregister_all()`. Both subcommands parse on all platforms; the dispatcher is `#[cfg(windows)]` for the actual call and prints "not supported on this platform" + exit 1 on non-Windows.
- [ ] `oriterm/src/main.rs`: Before `App::run()`, check for `-Embedding` in args (case-insensitive). If present (and on Windows): call `run_com_server()` which internally creates the channel, registers the COM factory, blocks until handoff arrives, and returns `HandoffData`. Pass that into `App::run_with_handoff()` (new method) which creates the event loop, window, tab, and adopted pane. On non-Windows, `-Embedding` is ignored (the CLI never receives it on those platforms).
- [ ] `oriterm/src/app/mod.rs`: Add `App::run_with_handoff(handoff: HandoffData) -> io::Result<()>` method that creates the winit `EventLoop`, a single window with one tab containing the adopted pane. Uses `oriterm_mux::domain::handoff::adopt_pane()` to create the pane. Applies startup info: `handoff.title` → pane title, `handoff.initial_rows/cols` → grid dimensions.
- [ ] `oriterm/src/app/tests.rs` (extend existing): semantic pin test that constructs a synthetic `HandoffData` with mocked reader/writer/signal and verifies `App::run_with_handoff()` creates a window and a single tab whose pane has the adopted reader/writer wired in. This test ONLY passes if the integration plumbs the handoff through correctly (cross-platform — uses headless GPU and no real COM activation).
- [ ] `oriterm/src/platform/mod.rs`: Add `#[cfg(windows)] pub(crate) mod default_terminal;` module declaration.
- [ ] Settings UI: "Set as default terminal" toggle in `oriterm/src/app/settings_overlay/form_builder/default_terminal/mod.rs` (directory module per test-organization rule) — add a toggle widget that calls `registry::register_all()` / `registry::unregister_all()`. Wire into the existing form builder module list. Add `oriterm/src/app/settings_overlay/form_builder/default_terminal/tests.rs` covering the toggle widget's handler in both the `register` and `unregister` directions (mocked registry layer).
- [ ] Guard: all code in `platform/default_terminal/` behind `#[cfg(target_os = "windows")]`. The adopt path in `oriterm_mux` is platform-independent (it accepts trait objects). CLI subcommands print "not supported" on non-Windows. The settings toggle is hidden on non-Windows.
- [ ] **Build verification**: `./build-all.sh && ./test-all.sh` (debug) + `cargo test --release` (release) at the END of Phase 4. Cross-compile must succeed for `x86_64-pc-windows-gnu` AND native Linux must build and test green.

**Phase 5 — Cargo.toml updates:**
- [ ] `oriterm/Cargo.toml` `[target.'cfg(windows)'.dependencies]`: Add missing `windows` crate features: `Win32_System_Com_Marshal`, `Win32_Security` (for `DuplicateHandle`). Add `windows-implement = "0.62"` for COM interface implementation macros. Add `windows-core = "0.62"` for `GUID` and `BSTR` types. Existing features (`Win32_System_Com`, `Win32_Foundation`, `Win32_System_Registry`) are already present.
- [ ] `oriterm_mux/Cargo.toml`: No changes needed. The adopt path uses `std::io::Read/Write` trait objects — no platform dependencies.
