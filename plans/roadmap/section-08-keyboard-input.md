...
- [x] **Mode flags** (5 progressive enhancement levels):
  ...
  - [ ] Bit 2 — `REPORT_ALTERNATE_KEYS` (4): report shifted/base key variants *(reopened 2026-03-29: mode flag stored in TermMode and can be pushed/popped, but `kitty.rs:encode_kitty()` never reads it -- encoding path not implemented. Alacritty implements this at `keyboard.rs:408-414`. Apps requesting flag 4 will get it on the stack but won't receive alternate key info.)*
