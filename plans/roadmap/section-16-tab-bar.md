...
- [x] Golden tests or scene capture tests for `Widget::draw()` output — `scene_to_snapshot()` in `oriterm_ui/src/testing/scene_snapshot/` converts Scene to deterministic text; 4 insta snapshot tests (`golden_single_tab`, `golden_three_tabs`, `golden_many_tabs_narrow_window`, `golden_active_tab_switch`) + determinism test + 14 snapshot formatting unit tests (added 2026-04-04)
