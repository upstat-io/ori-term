...
**Cluster collateral fix path (2026-04-25):** Implementing this subsection partially mitigates the BUG-06-016/018/019 notcurses-demo cluster — see `plans/bug-tracker/fix-BUG-06-020.md` for the full divergence chain. notcurses' XTGETTCAP `RGB` capability key sets `idata->rgb = true` independent of TN matching (`~/projects/reference_repos/console_repos/notcurses/src/lib/in.c:1733`), so an oriterm XTGETTCAP reply that includes `RGB=8/8/8` will recover RGB-required rendering. Full mitigation of quadrants/sextants/smulx capability flags requires upstream notcurses changes (adding `oriterm(` to `xtvers[]` and a `TERMINAL_ORITERM` heuristics branch) — out of scope for this tracker because notcurses' TN→qterm map at `in.c:1717-1731` only recognizes `xterm/mlterm/xterm-kitty/xterm-ghostty/xterm-256color`, so even an XTGETTCAP TN response of `oriterm` doesn't route through `apply_*_heuristics`.
