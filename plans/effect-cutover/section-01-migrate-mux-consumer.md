...
- [ ] Changes land in this order: 01.1 (atomic sink swap + router + new `MuxEvent`/`MuxNotification` variants — single commit; spans `oriterm_mux` **AND** `oriterm` because `oriterm/src/app/mux_pump/mod.rs` has an exhaustive `match` on `MuxNotification` that will not compile without new match arms for `HostClipboardLoad` and `HostColorQuery`; `oriterm_core` is NOT touched since effect types already exist) → 01.2 (idle-wake channel + response_poll activation + main-thread fulfill handlers — spans `oriterm_mux` + `oriterm`) → 01.3 (legacy deletions — spans `oriterm_core` + `oriterm_mux` + `oriterm` in a single commit because `LegacyEventSink` is referenced from `event_proxy/mod.rs`; compiler refuses any intermediate state) → 01.4 (daemon IPC audit — edits depend on chosen path).
