...
- `oriterm_mux/src/backend/embedded.rs` (or wherever `EmbeddedMux` is implemented) — implement `MuxBackend::fulfill_host_request` by looking up `pane_id` → `PaneIoHandle` in the registry and calling `handle.fulfill_clipboard_load(token, text)` or `handle.fulfill_color_query(token, color)`.
...
- [ ] **Semantic pin — THE critical test for this section** — `response_poll_idle_wake_unblocks_select` in `response_poll/tests.rs`. Setup: (1) Create a `crossbeam_channel::unbounded()` mux-event receiver pair to capture `MuxEvent` output. (2) Spawn `PaneIoThread<QueueingEffectSink>` via `PaneIoThread::spawn()`, passing the mux-event sender. (3) Send a command (via `handle.cmd_tx`) that causes the IO thread to push `Effect::HostRequest(HostRequest::ClipboardLoad { clipboard_char: b'c', terminator: "\x1b\\".into(), reply: token.clone() })` into the sink — the command is the canonical `TermCommand::ProcessBytes(bytes)` where `bytes` carries an OSC 52 read sequence; drive this through a real PTY byte injection, NOT a back-door direct `term.effect_sink().push(..)` call (which bypasses the VTE pipeline and doesn't exercise the production code path per `.claude/rules/tests.md §Test Real Code Paths`). (4) After the IO thread drains the PTY bytes, the token is registered but not fulfilled. (5) On the main thread, call `handle.fulfill_clipboard_load(&token, "hello".to_string())`. (6) Assert deterministically: call `mux_event_rx.recv_timeout(Duration::from_secs(5))` and assert it returns `Ok(MuxEvent::PtyWrite { data })` where `data` contains the base64-encoded `"hello"`. No `std::thread::sleep` — `recv_timeout` blocks until the event arrives or the 5-second deadline fires (a 5-second failure is a hard error, not a timing assertion). **Companion test** `response_poll_wake_is_load_bearing` (NOT via sleep) — same setup but do NOT call `fulfill_clipboard_load`; assert `mux_event_rx.try_recv()` returns `Err(TryRecvError::Empty)` immediately after the IO thread has processed the OSC 52 read bytes (synchronize the "thread has processed bytes" signal via a `TermCommand::Ping` round-trip or a `handle.cmd_tx` flush acknowledged by the mux-event sender emitting `MuxEvent::Output` — a deterministic handshake, not a sleep). This pins that `PtyWrite` is only emitted AFTER fulfillment, not speculatively.
