...
9. **`HostEffect::ChildExit` has a real production emitter** — pinned by `pty_eof_emits_pane_exited_via_effect_router`. The legacy `Event::ChildExit` ghost-variant path (matched in `event_proxy/mod.rs:143-148` and `mux_event/mod.rs:245` but never emitted by production code) is replaced by a `PaneIoThread`-side emitter fed by a dedicated child-watcher thread: at spawn time a `ChildExitWaiter` takes ownership of the `Box<dyn portable_pty::Child>` from `PtyHandle` and blocks on `child.wait()`, forwarding the `ExitStatus` on a `child_exit_rx: Receiver<ExitStatus>` paired into `PaneIoThread`. When either `byte_rx` closes or `child_exit_rx` fires, the IO thread pushes `Effect::Host(HostEffect::ChildExit { code })` with the real exit code and flushes the router before returning. The PTY reader thread owns only `Box<dyn io::Read>` (unchanged); `Pane`'s existing `PtyLifecycle` kill/signal path on `PtyHandle` is untouched. Without this, `MuxEvent::PaneExited` would never fire after 01.3 deletes the legacy path. Delivered by Section 01 (01.1).
