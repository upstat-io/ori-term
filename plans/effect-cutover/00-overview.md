...
9. **`HostEffect::ChildExit` has a real production emitter** — pinned by `pty_eof_emits_pane_exited_via_effect_router`. The legacy `Event::ChildExit` ghost-variant path (matched in `event_proxy/mod.rs:143-148` and `mux_event/mod.rs:245` but never emitted by production code) is replaced by a real `PaneIoThread`-side emitter: when `byte_rx` closes, the IO thread pushes `Effect::Host(HostEffect::ChildExit { code })` with the real exit code from the PTY reader's `child_exit_tx` channel before returning. Without this, `MuxEvent::PaneExited` would never fire after 01.3 deletes the legacy path. Delivered by Section 01 (01.1).
