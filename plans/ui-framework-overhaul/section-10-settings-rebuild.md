...
- [x] **Delete** the existing section builder functions: `build_appearance_section()`,
  `build_font_section()`, `build_behavior_section()`, `build_terminal_section()`,
  `build_bell_section()` from `form_builder/mod.rs`.
  **Done:** Replaced by per-page submodules (appearance.rs, colors.rs, font.rs, etc.).
- [x] **Delete** the `OPACITY_VALUES`, `FONT_SIZE_VALUES`, `BELL_DURATION_VALUES` constants
  ...
  **Done:** `OPACITY_VALUES` and `FONT_SIZE_VALUES` removed. `BELL_DURATION_VALUES` retained
  (bell dropdown still uses it).
- [x] **Rename** `build_settings_form()` to `build_settings_dialog()` (signature changes to
  accept `&UiTheme`).
- [x] **Replace** the old `SettingsIds` struct (10 fields) with the new one (22 fixed fields
  + `scheme_card_ids: Vec<WidgetId>` for per-card scheme selection).
  **Deviation:** `reset_defaults_id` not in `SettingsIds` — the Reset Defaults button is
  internal to `SettingsPanel` and emits `WidgetAction::ResetDefaults` directly.
  `scheme_card_grid` replaced by `scheme_card_ids: Vec<WidgetId>` since each SchemeCard has
  its own `WidgetId`.
...
- [x] Each page builder function wraps its content in `ScrollWidget::vertical()`.
  **Done:** All 8 page builders wrap content in `ScrollWidget::vertical()` with
  `set_height(SizeSpec::Fill)`.
- [x] Fix `PageContainerWidget::layout()` to use `SizeSpec::Fill` (not `Hug`) for the
  ...
  **Current issue**: `PageContainerWidget::layout()` returns a `LayoutBox::leaf(w, h)` where
  ...
  constrains it.
- [x] Test: Colors page with 10+ SchemeCards scrolls smoothly with scrollbar visible.
  **Done:** Colors page wrapped in `ScrollWidget::vertical()` with `SizeSpec::Fill` height.
  Visual verification deferred to Section 11 (no visual regression test infra yet).
...
- [x] Expand `SettingsIds` struct with all new control IDs.
  **Done:** 22 fixed `WidgetId` fields + `scheme_card_ids: Vec<WidgetId>` for per-card scheme
  selection. `reset_defaults_id` not needed (button internal to SettingsPanel).
- [x] Add `ValueChanged` and `TextChanged` to `handle_dialog_content_action()` match arm in
  `oriterm/src/app/dialog_context/content_actions.rs`.
  **Done:** All four action types (`Toggled`, `Selected`, `ValueChanged`, `TextChanged`)
  forwarded to `dispatch_dialog_settings_action()`. `key_conversion.rs` already extracted
  (459 lines, under 500).
- [x] Update `handle_settings_action()` to route all new control IDs to config fields.
  **Done:** 7 handler functions (`handle_appearance`, `handle_colors`, `handle_font`,
  `handle_terminal`, `handle_window`, `handle_bell`, `handle_rendering`) covering all
  control types: `Selected`, `Toggled`, `ValueChanged`, `TextChanged`. 21 tests passing.
- [x] **Overlay dispatch path is already correct**: `try_dispatch_settings_action()` in
  `overlay_dispatch.rs` is called BEFORE the match block in `handle_overlay_result()`, so ALL
  delivered actions pass through `handle_settings_action()`. No routing changes needed.
- [x] Add `Config` fields for new settings.
  **Done:** All fields exist: `font.line_height: f32`, `window.tab_bar_position: TabBarPosition`,
  `window.grid_padding: f32`, `window.restore_session: bool`, `rendering.gpu_backend: GpuBackend`.
  Added in 10.0b.
- [x] Update `apply_settings_change()` to handle new config fields.
  **Done:** `apply_font_changes()` detects `line_height` changes (added to font_changed check).
  `apply_color_changes()` handles scheme. `apply_cursor_changes()` handles cursor.
  `apply_window_changes()` handles opacity/blur. Settings like `tab_bar_position`,
  `grid_padding`, `restore_session`, `gpu_backend` are stored in config and read at usage sites
  (GPU backend requires restart per dropdown description).
- [x] **Config field name mapping** verified — all fields exist with correct paths.
- [x] Each new `Config` field has:
  struct definition with `#[serde(default)]`, Default impl, TOML round-trip tests in
  `oriterm/src/config/tests.rs`, and apply handler (or config-read-at-usage-site behavior).
- [x] Test: action handler tests cover all 22+ control IDs with `ValueChanged`, `TextChanged`,
  `Selected`, and `Toggled`. Config round-trip tests verify serialization.
...
- [x] `apply_font_changes()`: add `line_height` and `font_family` handling.
  **Done:** `line_height` added to `font_changed` detection in `config_reload/mod.rs`.
  `font_family` was already detected (`new.font.family != old.family`).
- [x] `apply_window_changes()`: `tab_bar_position`, `grid_padding`, `restore_session`,
  `columns`, `rows` are stored in config and read at usage sites. No hot-swap needed —
  `tab_bar_position`/`grid_padding` take effect when layout is recomputed,
  `restore_session`/`columns`/`rows` affect new windows.
- [x] GPU backend: saved to config, applies on next launch. Dropdown description says
  "restart required to apply". No `apply_rendering_changes()` needed.
- [x] Subpixel toggle writes to `config.font.subpixel_mode`, handled by existing
  `apply_font_changes()` (already triggers font re-rasterization).
- [x] `apply_cursor_changes()`: CursorPicker uses the same `CursorStyle` enum — no change
  needed.
- [x] Shell change (`terminal.shell`) — description text "Takes effect for new terminal tabs"
  already in `terminal.rs` `build_shell_section()`.
- [x] Scrollback change (`terminal.scrollback`): takes effect for new panes. Existing panes
  keep their current scrollback size (resizing an active scrollback buffer risks data loss).
...
### Layout & Navigation
- [x] Settings dialog opens at 860x620 with sidebar + content layout
- [x] Sidebar shows 8 nav items in 2 sections (General: 6 items, Advanced: 2 items)
- [x] Clicking a nav item switches to the corresponding page
- [x] Active nav item has accent highlight
- [x] All 8 pages render with correct content matching mockup
- [x] Both rendering modes work: overlay (modal in terminal window) and dialog (separate
  OS window) both show the new sidebar + pages layout correctly
- [x] Keyboard Tab cycles through controls on the active page
- [x] Arrow keys navigate the sidebar when it has focus

### Controls
- [x] All controls are functional (dropdowns, toggles, RangeSlider, NumberInputs, pickers)
- [x] SchemeCards show terminal preview + swatch bar, selection works
- [x] Palette editor shows SpecialColorSwatches + ANSI ColorSwatchGrids for active scheme
- [x] CodePreview updates when font settings change
- [x] CursorPicker shows 3 visual options with selection
- [x] Keybindings page shows all shortcuts with KbdBadge styling

### Scroll
- [x] Colors page scrolls when content exceeds viewport
- [x] Scrollbar is visible on pages with overflow content
- [x] Scroll position resets to top when switching pages
  **Done:** `Widget::reset_scroll()` trait method added (default no-op). `ScrollWidget`
  overrides to zero both offsets. `PageContainerWidget::accept_action(Selected)` calls
  `reset_scroll()` on the newly-active page.

### Persistence & Actions
- [x] Save persists all settings to config.toml
- [x] Cancel discards changes
- [x] Reset to Defaults restores all settings to `Config::default()`
- [x] Footer buttons styled correctly (ghost for Reset/Cancel, primary for Save)
- [x] `ValueChanged` and `TextChanged` actions route correctly in both overlay and dialog
  paths (not silently dropped)
- [x] Dirty state comparison works (`pending_config != original_config`)

### Config
- [x] All new `Config` fields have `#[serde(default)]` and round-trip TOML tests
- [x] `Config` and all nested config structs derive `PartialEq` (needed for dirty detection)
- [x] `TabBarPosition` serializes as `"top"` / `"bottom"` / `"hidden"`
- [x] `GpuBackend` serializes as `"auto"` / `"vulkan"` / `"dx12"` / `"metal"`
- [x] Existing config files without new fields load correctly (serde defaults)

### Tests
- [x] `form_builder/tests.rs` rewritten for the new 8-page sidebar structure
- [x] `action_handler/tests.rs` has tests for all new control IDs (`ValueChanged` for
  RangeSlider/NumberInputs, `TextChanged` for shell TextInput, `Selected` for new dropdowns
  and CursorPicker)
- [x] If page builders are extracted to submodules, each submodule with tests needs its own
  `tests.rs` sibling per test-organization.md rules
  **Note:** Page builders don't have their own tests — tested via `build_settings_dialog()`.
- [x] TOML round-trip tests for every new config field in `oriterm/src/config/tests.rs`
- [x] Test `WidgetAction::ResetDefaults` dispatch resets all config fields to defaults
  **Verified:** `reset_dialog_settings()` sets `Config::default()` and updates dirty state.
- [x] Test that `SettingsPanel::new()` (overlay) and `SettingsPanel::embedded()` (dialog)
  both produce valid layouts with sidebar + pages
- [x] `./test-all.sh` green, `./clippy-all.sh` green, `./build-all.sh` green

### File Size Compliance
- [x] `content_actions.rs` stays under 500 lines (458 lines; `key_conversion.rs` extracted)
- [x] `settings_panel/mod.rs` stays under 500 lines (399 lines)
- [x] Each page builder submodule stays under 500 lines (largest: colors.rs at 221 lines)
- [x] `form_builder/mod.rs` stays under 500 lines (199 lines)

### Legacy Code Cleanup
- [x] Old `build_appearance_section()`, `build_font_section()`, `build_behavior_section()`,
  `build_terminal_section()`, `build_bell_section()` deleted from `form_builder/mod.rs`
- [x] Old `OPACITY_VALUES`, `FONT_SIZE_VALUES` constants removed (replaced by RangeSlider
  and NumberInput controls)
- [x] No references to old `FormLayout`-based settings form remain (except in `_old/`)
