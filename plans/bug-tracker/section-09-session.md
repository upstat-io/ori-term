...
- [ ] `[BUG-09-3][low]` **No integration test for cell-metric broadcast short-circuit + cross-window pane seeding** — found by tpr-review.
  Repro: N/A — test-coverage gap, not a behavioral bug.
  Detail: `App::broadcast_cell_metrics_to_window`'s short-circuit (keyed by `WindowContext::last_broadcast_cell_dims`) and `App::seed_pane_with_window_cell_metrics` (called from `move_tab_to_window` and `tear_off_tab`) are unit-tested at the pure-helper level (`oriterm/src/app/cell_metrics/tests.rs::cell_metric_broadcast_needed`). Full integration tests verifying (1) the stateful cache assignment `ctx.last_broadcast_cell_dims = Some(...)` actually fires, and (2) the seed helper is correctly invoked from the pane-move call sites, require an App fixture with a mux + window + session that does not yet exist in the test infrastructure.
  Impact: A future refactor that removes the cache-write assignment in `broadcast_cell_metrics_to_window` or removes the `seed_pane_with_window_cell_metrics` calls from move/tear-off paths would pass the existing pure-helper tests but silently regress the optimization / correctness guarantee. The pure-helper tests pin the decision rule but not the wire-up.
  Proposed fix: Either (a) add an `App` test fixture under `oriterm/src/app/test_support.rs` that constructs a minimal `App` + `EmbeddedMux` + `WindowContext` + `Session` without GPU, suitable for unit-testing the broadcast and seed behavior; or (b) add an architecture test under `oriterm/tests/architecture.rs` that greps for the assignment and call-site patterns (lightweight but catches accidental removal). Option (a) is the proper fix; option (b) is a stopgap.
  Subsystem: `oriterm/src/app/cell_metrics/`, `oriterm/src/app/test_support.rs`, `oriterm/tests/architecture.rs`
  Found: 2026-04-13 | Source: tpr-review | Reviewers: codex (TPR-07-001-codex) + gemini (TPR-07-003-gemini) (agreement — both flagged the same test gap)
  Note: The pure-helper `cell_metric_broadcast_needed` IS tested (6 cases pinning the decision rule). The gap is at the stateful / call-site layer.
