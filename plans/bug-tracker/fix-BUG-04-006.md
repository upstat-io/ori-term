...
- [ ] `subpixel_bitmap_matches_raw_swash_output_not_lut_image` (terminal grid, RGB): `FontCollection::rasterize` for 'H' as `GlyphFormat::SubpixelRgb` emits a bitmap byte-equal to calling `rasterize_from_face` directly with the same parameters — i.e. swash's raw per-subpixel coverage, NOT the LUT image. Construction: call `rasterize_from_face(...)` in `oriterm/src/font/collection/face.rs` to obtain the raw reference bitmap; compare byte-for-byte against the bitmap returned by `FontCollection::rasterize`. Assert `raw == cached`. On HEAD, cached is the LUT'd form → fails; after the fix, cached is raw → passes.
- [ ] `subpixel_bitmap_matches_raw_swash_output_not_lut_image_bgr`: same construction for `GlyphFormat::SubpixelBgr` — covers the BGR path via `Format::subpixel_bgra()`.
- [ ] `subpixel_bitmap_ui_text_path_matches_raw`: same construction for `FontCollection::rasterize_with_weight` (UI text weight-aware path). Bug exists in both functions; both need independent red-first coverage.
- [ ] `subpixel_mean_coverage_lower_than_gamma_boosted` (user-visible weight pin): mean of `SubpixelRgb` bitmap (R+G+B bytes only) for 'H' is STRICTLY LESS than `apply_alpha_correction(&mut glyph, &lut)` applied to that same bitmap (i.e. the LUT image of the current output). On HEAD, these are equal → fails; after the fix, the current output is the raw form and LUT'ing it produces a larger mean → passes.

### Invariant / regression guards (green on HEAD, catch future refactors)

- [ ] `alpha_bitmap_still_gamma_boosted` (grayscale regression pin): rasterize 'H' as `GlyphFormat::Alpha` via `rasterize`; obtain the raw swash output via `rasterize_from_face`; assert mean coverage of the rasterized result is STRICTLY GREATER than mean coverage of the raw. Green on HEAD and after the fix; fires if someone accidentally widens the new guard to include Alpha.
- [ ] `alpha_bitmap_ui_text_path_still_gamma_boosted`: same construction for `rasterize_with_weight` on `GlyphFormat::Alpha`. Pins the weight-aware Alpha guard independently so a refactor that only touches `rasterize` cannot silently regress the UI path.
- [ ] `subpixel_bitmap_alpha_channel_zero`: every 4th byte of a `SubpixelRgb` bitmap equals exactly 0 (zeno never writes the A channel; LUT[0] = 0, so the invariant holds either way). Catches a regression where someone starts writing to the A channel or flips LUT[0].

(No direct `apply_alpha_correction` white-box test: the function is private in `rasterize.rs` — bundling a visibility widening into this fix is scope creep, and the grayscale behavior is already exercised by `alpha_bitmap_still_gamma_boosted` + `alpha_bitmap_ui_text_path_still_gamma_boosted` through the public `rasterize` / `rasterize_with_weight` entry points. If a direct-call pin is later needed, widen `apply_alpha_correction` to `pub(super)` in a separate follow-up.)

### Cross-feature / synthetic-bold interaction

- [ ] `synthetic_bold_subpixel_vs_regular_weight`: rasterize 'H' as Synthetic-Bold in `SubpixelRgb` and compare mean R+G+B coverage against 'H' as Regular in `SubpixelRgb`. Pin `bold_mean > regular_mean`. Catches the `embolden_strength()` interaction from §1.5 (multiplier reduced from 2.0 → 1.5 at `oriterm/src/font/collection/face.rs:249-259` specifically to compensate for the gamma correction subpixel will no longer receive). Pre-condition: verify the embedded test font (`JetBrainsMono-Regular.ttf`) has no native Bold face so `GlyphStyle::Bold` routes through synthetic. If a native bold face is present the test MUST assert synthetic-bold path is taken (e.g. via a `SyntheticFlags::BOLD` check on the resolved key) — otherwise the matrix cell is testing native bold and the embolden_strength interaction is unexercised. If the embedded corpus cannot be forced through synthetic, swap to a font in `oriterm/src/font/assets/` with Regular-only coverage OR construct the scenario via `rasterize_with_weight` bypassing face resolution.
...
- [ ] Run the GPU visual regression suite under the lib tests (the only integration target in `oriterm/tests/` is `architecture`; visual regression lives under `oriterm/src/gpu/visual_regression/`):
  ...
  ```
  Before accepting the overwrite, run the §2 "Pre-overwrite semantic pre-assertion" (R-channel mean-coverage decrease + per-channel asymmetry bound). Refuse the re-baseline if the assertion fails. List every moved golden in the commit body with its byte-count delta so the re-baseline is auditable.
...
- [ ] Matrix completeness verified — RGB + BGR subpixel paths, terminal + UI-text rasterizers, grayscale regression pin, color-emoji regression pin
