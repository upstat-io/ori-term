...
- [ ] `[BUG-08-17][medium]` **`Cell` lacks a CHARDRAWN-equivalent flag — DECRQCRA (CSI * y) silently skips application-written spaces with default SGR** — found by /tpr-review round 2 on spec-conformance §09A.5 (DECRQCRA).
  Repro: On a fresh 3×3 terminal, have the application write `"A B"` at row 0 (so `'A'` at col 0, `' '` at col 1, `'B'` at col 2, all with default SGR). Send `CSI 1 ; 1 ; 1 ; 1 ; 1 ; 3 * y`. The DCS reply omits the middle `' '` contribution (treated as undrawn): ori_term returns `-(0x41 + 0x42) = 0xFF7D`. xterm returns `-(0x41 + 0x20 + 0x42) = 0xFF5D`. 32-byte checksum divergence.
  Subsystem: `oriterm_core/src/cell/mod.rs` (new `CellFlags::DRAWN` bit + mutator wiring), `oriterm_core/src/grid/editing/mod.rs` (set DRAWN in `put_char_ascii` / `put_char_slow` / spacers / LEADING_WIDE_CHAR_SPACER), `oriterm_core/src/term/handler/rect_ops/mod.rs::compute_rect_checksum` (consult `CellFlags::DRAWN` instead of `!cell.is_empty()`).
  Analysis: `Cell::is_empty()` returns true for any cell whose character is `' '` with default fg/bg/flags/extra. That conflates "never written" (CHARDRAWN=0 in xterm) with "application wrote a literal space under default SGR" (CHARDRAWN=1). xterm's `xtermCheckRect` at `~/projects/reference_repos/console_repos/xterm/screen.c:3178-3180` uses `ld->attribs[col] & CHARDRAWN` to skip undrawn cells and at `screen.c:3236` uses `DRAWX_MASK = (ATTRIBUTES | CHARDRAWN)` to push drawn cells (even drawn blanks) into `trimmed`. Our `drawn = !cell.is_empty()` proxy is correct for the common "pristine vs. written-with-attrs" cases but wrong for the "written-as-plain-space" case.
  Scope: Bigger than a §09A.5 subsection — the DRAWN bit must be set on every cell-write path (`put_char_ascii`, `put_char_slow` main cell + spacer + LEADING_WIDE_CHAR_SPACER, `push_zerowidth` target cell) and cleared on every reset path (`Cell::reset`, `Row::reset`, `clear_range`, `truncate`, ED/EL erase operations). Existing tests that construct `Cell { ch: 'A', ..Cell::default() }` directly (including rect_ops tests) would need `flags: CellFlags::DRAWN` added. Downstream implication: `is_empty()` definition may need to incorporate DRAWN to keep its existing "visually empty" semantics stable.
  TDD matrix: (a) `put_char(' ')` under default SGR produces `DRAWN == true`; (b) freshly created Row has all cells `DRAWN == false`; (c) `clear_range` clears DRAWN; (d) DECRQCRA on a grid with explicit-space content matches xterm byte-for-byte; (e) reflow preserves DRAWN across shrink + grow; (f) scrollback eviction resets DRAWN via `Row::reset`; (g) insert_blank / delete_chars preserve DRAWN on shifted cells and clear it on freshly-inserted blanks.
  /tpr-review round-2 evidence: codex round 2 F1 — `Explicit default spaces are treated as undrawn` — cited verbatim by the review with xterm source reference.
