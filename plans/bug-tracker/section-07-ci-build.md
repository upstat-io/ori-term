...
- [ ] `[BUG-07-020][medium]` **`vttest_available()` probe `vttest --help` exits 1 — entire vttest text-side suite (8 menu test files × 3 sizes = 24 tests) silently skips on every host** — found during /fix-bug BUG-07-007 Phase 3 (2026-04-25).
  ...
  Severity: medium — the bug is invisible (silent skip, not a panic) but it makes the entire vttest text-side conformance suite (200+ insta snapshots) DARK on every dev/CI host that has vttest installed. Snapshot drift cannot be detected, regressions cannot be caught.
  Fix: change `vttest_available()` from `tool_available("vttest", "--help")` to `tool_available("vttest", "-V")`. Verified: `vttest -V` prints `VT100 test program, version 2.7 (20230201)` and exits `0`. Update the doc comment to reflect that vttest, like tack, is "the odd one out" — the convention `--version`/`--help` doesn't apply.
  Note: discovered during /fix-bug BUG-07-007 Phase 3 when the new walker's smoke tests reported "vttest not installed, skipping" on a host with `/usr/bin/vttest` present. This is a fix-interference blocker for BUG-07-007 — without this fix, BUG-07-007's smoke tests cannot validate the migration. Fixed inline in the BUG-07-007 commit per /fix-bug fix-interference reordering.
