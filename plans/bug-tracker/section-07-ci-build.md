...
- [ ] `[BUG-07-022][medium]` **SSOT-alignment matrix and producer-shape contract not extended to CSI/DCS/ESC/APC categories** — found by /impl-hygiene-review during /fix-bug BUG-07-019 Phase 5 (run_id `/tmp/impl-hygiene-ori_term-RtDFbAFA`, Phase 3 findings F-05 + F-07 + F-11 cluster).
  Escalated: requires plan — multi-phase scope (4 categories × ~150-200 lines per matrix + cross-producer SSOT reconciliation for csi_params_placeholder extracted from capture_extract.rs to canonical.rs and consumed by 3 producer paths). Affects 5+ files (canonical.rs new SSOT helper, dispatch_extract/csi.rs + dcs walker + esc walker + apc handling, capture_extract.rs, uncataloged/mod.rs, tests.rs matrix expansion). Each category needs its own osc_ssot_matrix-equivalent + per-producer params-shape assertions. Naturally phases as: phase 1 SSOT helper extraction → phase 2 CSI matrix → phase 3 DCS matrix → phase 4 ESC matrix → phase 5 APC matrix → phase 6 producer-shape SSOT reconciliation. Escalated by /fix-bug --autopilot Phase 1.5 (2026-04-25). Plan creation needed: `plans/test-coverage-ssot-expansion/` or similar. The user creates the plan after the autopilot run completes per /fix-bug autopilot escalation protocol.
