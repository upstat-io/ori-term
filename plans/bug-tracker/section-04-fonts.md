...
- [x] `[BUG-04-005][high]` **Bold text no longer renders — regression** — found by manual.
  Resolved: 2026-04-01. Could not reproduce — 34 bold tests pass, font pipeline confirmed correct. User confirmed bold works after rebuild. Likely stale binary.

- [x] `[BUG-04-004][medium]` **Emoji in tab title vanishes after monitor transition** — found by manual.
  Resolved: fixed on 2026-04-25. Root cause: `UiFontSizes::rebuild_all` (called by `set_dpi` on DPI change, and by `ensure_size` / `create_default_collection` on runtime size additions) reconstructed every `FontCollection` from `font_set.clone()` — a `FontSet::ui_embedded()` with empty `fallbacks` — and the `post_rebuild_hook` only re-applied user font config. The emoji fallback injected once at `WindowRenderer::new` via `sizes.inject_fallbacks(&emoji_data)` was not persisted on `UiFontSizes`, so every rebuild dropped it. A secondary path (config reload) had the same gap — `rebuild_ui_font_sizes` created a fresh registry that was never re-injected. Fix is a two-layer SSOT: (1) `UiFontSizes` gained an `injected_fallbacks: Vec<FontData>` field with idempotent `inject_fallbacks` (Arc::ptr_eq + index dedup) and a private `finalize_collection` helper that replays the injection on every rebuild path; (2) `WindowRenderer::replace_font_collection` gained a `reinject_emoji_fallback()` call that re-wires emoji whenever the terminal font changes. 10 regression tests (9 unit + 1 GPU). Commits: `3c510928` (initial SSOT) + `3528b508` (source-side ordering) + `73ac90c8` (GPU regression test) + `e6fc75c8` (Arc::ptr_eq pin) + `dd0012d0` (TPR round log) + this closure commit. Fix section: `plans/bug-tracker/fix-BUG-04-004.md`. Reviews: `/tp-help` (design consensus), 4x `/tpr-review` (exit reason: clean), `/impl-hygiene-review` (resolved inline, filed pre-existing `BUG-06-021`).
