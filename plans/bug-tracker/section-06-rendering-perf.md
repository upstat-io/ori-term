...
- [ ] `[BUG-06-012][high]` **notcurses-demo reports ~5 FPS drops that did not occur before Section 07 work**
  ...
  Found: 2026-04-14 | Source: manual
  **Blocked:** pending BUG-06-019 fix and/or profiling data — see Investigation 2026-04-24 below for search-space narrowing.
  Investigation 2026-04-24 (fix-BUG-06-012 Phase 1, /fix-bug exited via Blocked path):
    ...
    - **Shared-scene hypothesis:** BUG-06-019 (upper rows of xray's 10-row marquee render dark/colorless) is also in the xray scene. A slow or broken color/transparency path could explain both symptoms — a dead-code color dispatch would both render wrong AND eat budget. If BUG-06-019's fix lands, re-measure BUG-06-012 before resuming investigation here.
    - **Blocking rationale:** diagnosing a 0.7% real-time deficit via code reading alone is guesswork; the kitty streaming path has too many candidate hot spots (VTE APC parsing, base64 decode — see BUG-08-023, image cache insertion, snapshot dirty tracking, GPU texture upload). Need either (a) a flamegraph / `cargo flamegraph --bin oriterm --release -- <notcurses-demo x>` run, or (b) BUG-06-019 fix landing so we can re-measure whether the two bugs shared a root cause. Resumption paths in order of preference: (1) fix BUG-06-019, re-measure; (2) user produces flamegraph of xray on a fresh run; (3) synthetic microbench streaming kitty APC frames at 57 fps to `Term` directly.
