...
- [ ] `[BUG-06-017][high]` **notcurses-demo `yield` demo (`y`): entire demo blips on screen then vanishes — demo exits early before running** — found by manual.
  ...
  Subsystem: most likely `oriterm_core/src/term/handler/image/kitty/` reply paths OR `oriterm_mux/src/pane/io_thread/effect_router/` routing — a malformed reply to notcurses's kitty-graphics probe (`\x1b_Gi=0\x1b\\`) OR to DA2 / device queries could be interpreted by `demo_getc` as spurious user input, causing early termination. The §13.0.5 BUG-08-7/8 closure changed kitty reply paths; §12 sixel added DA2-related handling. Alternative: if ori_term's kitty-graphics capability probe reply is wrong, notcurses falls back through blitter chain (kitty → sixel → sextant → ...) and may emit a `canopen_images`-style early exit. Fix location TBD via `/fix-bug` Phase 1 root-cause analysis — capture the kitty+DA2 reply stream on ori_term and compare against wezterm/kitty reference.
  Found: 2026-04-22 | Source: manual
  Note: Sibling to BUG-06-016, BUG-06-018. Related to existing BUG-06-012 (~5 FPS drops post-§07). This is a DIFFERENT symptom class from 016/018 (glyph corruption) — early termination points at reply-path / capability-probe pollution, not rendering correctness. May have separate root cause; confirm during investigation.

- [ ] `[BUG-06-016][high]` **notcurses-demo `mojibake` demo (`m`): a vertical column range immediately to the right of scrolling emoji planes stays BLACK — stdplane background text never re-renders onto the cells the emojis vacated** — found by manual.
