...
- [ ] `[BUG-06-022][medium]` **XTSMGRAPHICS (`CSI ? Pi ; Pa ; Pv S`) graphics geometry / color-register query has no reply path — notcurses queries for sixel parameters and gets nothing** — found by 2026-04-25 BUG-06-020 diagnostic.
  Escalated: 2026-04-25 — fix is owned by `plans/roadmap/section-39-image-protocols.md` §39.10 "XTSMGRAPHICS Sixel Size Negotiation" (status: `not-started`). The bug entry's own subsystem note already declares the routing; this Escalated line formalizes it for `/fix-next-bug` queue exclusion (matches BUG-06-017 / BUG-11-011 pattern). Parent section 39 is `reviewed: false` — implementation gates on `/review-plan` flipping the section, after which `/continue-roadmap` picks up §39.10. The §39.10 body already has the required-work checklist (CSI ? 1 ; 1 ; 0 S → max graphics dimensions; CSI ? 2 ; 1 ; 0 S → max color register count; response format `CSI ? Pi ; 0 ; Pv S`). No fix-section file is created — `/fix-bug` is not the execution vehicle for plan-owned work. Bug entry stays open as the tracker pointer to §39.10.
