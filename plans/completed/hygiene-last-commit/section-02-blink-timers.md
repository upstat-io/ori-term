...
- [x] Extract a private helper struct and computation method. The tuple `(f64, f64, f64)` proposed originally is insufficient because `next_change()` needs `cycle_start_secs` and `in_secs`, and `intensity_at()` needs `fade_out_dur` and `fade_in_dur`. Use a struct:
  ```rust
...
- [x] Add a private method that computes the struct:
  ```rust
...
- [x] Refactor `intensity_at()` to call `phase_bounds()`. The `None` (zero-duration) case returns `1.0` (existing behavior). The phase match uses `pb.fade_out_start`, `pb.fade_out_end`, `pb.hidden_plateau_end`, `pb.fade_out_dur`, and `pb.fade_in_dur`.

- [x] Refactor `next_change()` to call `phase_bounds()`. The `None` case returns `self.epoch + Duration::from_secs(1)` (existing behavior). The phase match uses `pb.cycle_start_secs` to compute absolute instants for plateau-end boundaries, and returns `Instant::now() + ANIMATION_FRAME_INTERVAL` for fade regions. <!-- reviewed: next_change needs cycle_start_secs for absolute Instant computation — this field was missing from original tuple proposal -->

- [x] Refactor `is_animating()` to call `phase_bounds()`. The `None` case returns `false` (existing behavior). The animating check is `(cycle_pos >= fade_out_start && cycle_pos < fade_out_end) || cycle_pos >= hidden_plateau_end`. <!-- reviewed: is_animating checks cycle_pos < in_secs (= fade_out_end), not cycle_pos < in_secs from a separate local — the struct provides this as fade_out_end -->

- [x] Ensure existing `cursor_blink/tests.rs` still passes (19 tests after 02.4 removed `next_toggle` test). <!-- reviewed: accuracy fix — was 20, now 19 after 02.4 -->
...
- [x] Add `use std::sync::atomic::AtomicBool;` import to `mod.rs` (in the `std` import group at line 52, alongside the existing `use std::sync::Arc;`). <!-- reviewed: accuracy fix — AtomicBool not currently imported in mod.rs -->

- [x] Add field to `App` in `mod.rs`:
  ```rust
...
- [x] Add `use std::sync::atomic::AtomicBool;` import to `constructors.rs` (in the `std` import group at line 4, alongside the existing `use std::sync::Arc;`). <!-- reviewed: accuracy fix — AtomicBool not currently imported in constructors.rs -->

- [x] Initialize in `constructors.rs` (around line 130, near the other blink fields):
  ```rust
  blink_wakeup_pending: Arc::new(AtomicBool::new(false)),
  ```

- [x] Update `schedule_blink_wakeup()` in `event_loop_helpers/mod.rs` to check and set the guard:
  ```rust
...
- [x] Add required imports to `event_loop_helpers/mod.rs`: `use std::sync::atomic::Ordering;`. No `Arc` import needed — `.clone()` works via method resolution on the `Arc<AtomicBool>` field. <!-- reviewed: accuracy fix — clarified that Arc import is not needed -->
...
- [x] Remove the `text_blink_active: bool` field from `ControlFlowInput` (line 338 of `event_loop_helpers/mod.rs`). Also remove its doc comment (line 337).

- [x] Update the doc comment on `next_text_blink_change` (line 339 after removal) — currently says "only meaningful if `text_blink_active`", should say "Next text blink phase boundary (always active — unconditional timer)." <!-- reviewed: accuracy fix — the doc comment references the removed field -->

- [x] Remove the `text_blink_active: true` assignment from the `ControlFlowInput` construction in `event_loop.rs` (line 478).

- [x] Update `compute_control_flow()` in `event_loop_helpers/mod.rs` to always consider text blink. The current logic:
  ```rust
...
- [x] Verify the updated `compute_control_flow()` logic handles all combinations correctly:
...
- [x] If clippy/build errors on `ControlFlowDecision::Wait` as dead code, add `#[allow(dead_code, reason = "reserved for future use when all timers can be disabled")]` on the `Wait` variant. Check by running `./build-all.sh` after removing the field. <!-- reviewed: guard step for dead_code = "deny" -->

- [x] Update `idle_input()` in `event_loop_helpers/tests.rs`: remove the `text_blink_active: false` field.

- [x] Update the `idle_returns_wait` test (tests.rs:24) — this test asserts `ControlFlowDecision::Wait` for the idle case, but with text blink unconditional, idle now returns `WaitUntil(next_text_blink_change)`. Update the assertion:
  ```rust
  #[test]
...
- [x] Remove `next_toggle()` from `cursor_blink/mod.rs` (lines 131-136).
- [x] Remove the test `next_toggle_delegates_to_next_change` from `tests.rs` (lines 284-298, including the `#[test]` attribute). <!-- reviewed: accuracy fix — line 284 is the #[test] attribute, 285 is the fn -->
- [x] Verify no other callers exist via grep (already confirmed — only plan files and the test reference it). <!-- reviewed: verified via grep — only plan docs, the method def, and one test call it -->
...
- [x] `[TPR-02-003][medium]` `oriterm/src/app/event_loop_helpers/mod.rs:243-272`, `oriterm/src/app/event_loop.rs:490-495`, `oriterm/src/app/redraw/post_render.rs:37-49`, `oriterm/src/app/keyboard_input/mod.rs:270-287` — the new `blink_wakeup_pending` guard deduplicates threads, but it also makes the wakeup deadline non-reschedulable. Once one sleeper is pending, later `about_to_wait` passes cannot replace it with an earlier deadline after `cursor_blink.reset()` or other blink-state changes. On the platforms this helper exists for (`ControlFlow::WaitUntil` unreliable on Windows/WSL2), that can delay the next blink phase transition until the stale sleeper fires, stretching the visible plateau after typing, focus changes, or cursor-motion resets.
  Resolved: Replaced `AtomicBool` guard with `AtomicU64` generation counter so reset paths can respawn with a fresh deadline and stale threads cannot clear a newer guard. Follow-up review found that stale sleepers still remain alive and can overlap after repeated resets; see `TPR-02-005`.

- [x] `[TPR-02-004][medium]` `oriterm/src/app/config_reload/mod.rs:301-312`, `oriterm/src/app/event_loop_helpers/mod.rs:257-291` — interval changes do not invalidate the currently pending wakeup thread. `apply_cursor_changes()` updates `cursor_blink` / `text_blink` durations in place, but it never clears `blink_wakeup_gen` or otherwise reschedules the sleeper. If a wakeup for the old interval is already pending, the next `schedule_blink_wakeup()` returns early on the nonzero generation and the old deadline remains in force until that stale thread fires. Shorter intervals therefore take effect late after config reload, and the same stale-sleeper problem exists for text blink.
  Resolved: Added `blink_wakeup_gen.store(0)` after both `cursor_blink.set_interval()` and `text_blink.set_interval()` in `apply_cursor_changes()`. Validated on 2026-04-03.

- [x] `[TPR-02-005][medium]` `oriterm/src/app/event_loop_helpers/mod.rs:243-245`, `oriterm/src/app/event_loop_helpers/mod.rs:279-290`, `oriterm/src/app/keyboard_input/mod.rs:285-286`, `oriterm/src/app/redraw/post_render.rs:37-48` — the `AtomicU64` generation guard does not actually guarantee a single pending wakeup thread across resets. `reset_cursor_blink()` zeroes the shared generation, but it does not cancel the thread that is already sleeping, and that stale thread still sends `MuxWakeup` even when its `compare_exchange` fails. After repeated keypress, focus, or cursor-move resets, `about_to_wait()` can spawn a fresh sleeper each time while the older sleepers remain alive, so overlapping wakeup threads still accumulate and later all wake the loop.
  Resolved: Changed `let _ = gen_ref.compare_exchange(...)` to `if gen_ref.compare_exchange(...).is_ok() { sender.send(...) }` — stale threads whose CAS fails now exit silently without sending MuxWakeup. Only the active-generation thread wakes the event loop. Validated on 2026-04-03.

---

## 02.N Completion Checklist

- [x] `CursorBlink` phase boundary computation exists in exactly one private method (`phase_bounds()`) returning a `PhaseBounds` struct
- [x] `schedule_blink_wakeup` has an `Arc<AtomicU64>` generation guard — stale threads' CAS fails so only the active generation sends MuxWakeup; config reload and blink resets clear gen for immediate rescheduling
- [x] `text_blink_active` field removed from `ControlFlowInput`; text blink timer unconditionally contributes to `compute_control_flow()`
- [x] `idle_returns_wait` test renamed to `idle_returns_text_blink_wait` and updated to expect `WaitUntil` (not `Wait`); `not_still_dirty_goes_idle` removed (redundant) <!-- reviewed: consistency fix — matches 02.3 decision to remove redundant test -->
- [x] `next_toggle()` removed from public API and its test removed
- [x] `./test-all.sh` green
- [x] `./build-all.sh` green
- [x] `./clippy-all.sh` green
- [x] `/tpr-review` passed (3 rounds: TPR-001/002 pre-existing, TPR-003/004/005 found and fixed)
