...
- [x] `[TPR-05-005][medium]` [plans/teseq-conformance/section-05-sgr-colors.md:715](/home/eric/projects/ori_term/plans/teseq-conformance/section-05-sgr-colors.md#L715), [plans/teseq-conformance/section-05-sgr-colors.md:881](/home/eric/projects/ori_term/plans/teseq-conformance/section-05-sgr-colors.md#L881), [oriterm_core/tests/teseq/sgr/resets.rs:168](/home/eric/projects/ori_term/oriterm_core/tests/teseq/sgr/resets.rs#L168), [oriterm_core/tests/teseq/sgr/combinations.rs:74](/home/eric/projects/ori_term/oriterm_core/tests/teseq/sgr/combinations.rs#L74) — The two tests that are supposed to pin reset-to-default color behavior only check flag clearing, so regressions that leave foreground/background colors behind after `SGR 0` or parameterless `CSI m` would still pass this section.
  Validation: `reset_sgr0()` verifies the styled cell's bright-red/green colors, but the post-reset `"Clean"` cell is only checked for three cleared flags and never compared against default fg/bg. `combo_empty_sgr_resets()` is even narrower: it only checks that bold/italic were cleared on `"Plain"`, not that the preceding red foreground was reset. The plan text explicitly claims those scenarios assert default colors after reset, but the current code does not, so Section 05 no longer proves the color-reset portion of `Attr::Reset`.
