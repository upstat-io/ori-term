...
pub(super) fn draw_block(canvas: &mut Canvas, ch: char) -> bool {
    ...
    match ch {
        // Upper half block.
        '\u{2580}' => canvas.fill_rect(RectF::new(0.0, 0.0, w, (h / 2.0).round()), 255),
        // Lower N/8 blocks (U+2581–U+2587).
        '\u{2581}'..='\u{2587}' => {
            let eighths = (ch as u32 - 0x2580) as f32;
            let bh = (h * eighths / 8.0).round();
            canvas.fill_rect(RectF::new(0.0, h - bh, w, bh), 255);
        }
        // Full block.
        '\u{2588}' => canvas.fill_rect(RectF::new(0.0, 0.0, w, h), 255),
        // Left N/8 blocks (U+2589–U+258F): 7/8 down to 1/8.
        '\u{2589}'..='\u{258F}' => {
            let eighths = (0x2590 - ch as u32) as f32;
            canvas.fill_rect(RectF::new(0.0, 0.0, (w * eighths / 8.0).round(), h), 255);
        }
        // Right half.
        '\u{2590}' => {
            let hw = (w / 2.0).round();
            canvas.fill_rect(RectF::new(w - hw, 0.0, hw, h), 255);
        }
        // Shade blocks: 25%, 50%, 75%.
        '\u{2591}' => canvas.fill_rect(RectF::new(0.0, 0.0, w, h), 64),
        '\u{2592}' => canvas.fill_rect(RectF::new(0.0, 0.0, w, h), 128),
        '\u{2593}' => canvas.fill_rect(RectF::new(0.0, 0.0, w, h), 191),
        // Upper 1/8.
        '\u{2594}' => canvas.fill_rect(RectF::new(0.0, 0.0, w, (h / 8.0).round()), 255),
        // Right 1/8.
        '\u{2595}' => {
            let bw = (w / 8.0).round();
...
fn draw_quadrant(canvas: &mut Canvas, ch: char) {
    ...
    if mask & 0b1000 != 0 {
        canvas.fill_rect(RectF::new(0.0, 0.0, hw, hh), 255);
    }
    if mask & 0b0100 != 0 {
        canvas.fill_rect(RectF::new(hw, 0.0, w - hw, hh), 255);
    }
    if mask & 0b0010 != 0 {
        canvas.fill_rect(RectF::new(0.0, hh, hw, h - hh), 255);
    }
    if mask & 0b0001 != 0 {
        canvas.fill_rect(RectF::new(hw, hh, w - hw, h - hh), 255);
    }
}
