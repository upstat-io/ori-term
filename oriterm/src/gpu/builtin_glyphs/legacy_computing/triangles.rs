    ...
    match edge {
        Edge::Left => {
            ...
            for r in 0..rows {
                ...
                canvas.fill_rect(RectF::new(0.0, r as f32, fw, 1.0), 255);
            }
        }
        Edge::Right => {
            let rows = h.ceil() as u32;
            for r in 0..rows {
                ...
                let dist = ((r as f32 + 0.5) - mid).abs() / mid;
                let fw = (w * (1.0 - dist)).round();
                canvas.fill_rect(RectF::new(w - fw, r as f32, fw, 1.0), 255);
            }
        }
        Edge::Top => {
            ...
            for c in 0..cols {
                ...
                canvas.fill_rect(RectF::new(c as f32, 0.0, 1.0, fh), 255);
            }
        }
        Edge::Bottom => {
            let cols = w.ceil() as u32;
            for c in 0..cols {
                ...
                let dist = ((c as f32 + 0.5) - mid).abs() / mid;
                let fh = (h * (1.0 - dist)).round();
                canvas.fill_rect(RectF::new(c as f32, h - fh, 1.0, fh), 255);
            }
        }
    }
}
...
fn fill_edge_complement(canvas: &mut Canvas, edge: Edge) {
    ...
    canvas.fill_rect(RectF::new(0.0, 0.0, w, h), 255);
    ...
    match edge {
        Edge::Left => {
            ...
            for r in 0..rows {
                ...
                canvas.fill_rect(RectF::new(0.0, r as f32, cw, 1.0), 0);
            }
        }
        Edge::Right => {
            let rows = h.ceil() as u32;
            for r in 0..rows {
                ...
                let dist = ((r as f32 + 0.5) - mid).abs() / mid;
                let cw = (w * (1.0 - dist)).round();
                canvas.fill_rect(RectF::new(w - cw, r as f32, cw, 1.0), 0);
            }
        }
        Edge::Top => {
            let cols = w.ceil() as u32;
            for c in 0..cols {
                ...
                canvas.fill_rect(RectF::new(c as f32, 0.0, 1.0, ch), 0);
            }
        }
        Edge::Bottom => {
            let cols = w.ceil() as u32;
            for c in 0..cols {
                ...
                let dist = ((c as f32 + 0.5) - mid).abs() / mid;
                let ch = (h * (1.0 - dist)).round();
                canvas.fill_rect(RectF::new(c as f32, h - ch, 1.0, ch), 0);
            }
        }
    }
}
...
fn fill_corner(canvas: &mut Canvas, ox: f32, oy: f32, dw: f32, dh: f32) {
    ...
    for r in 0..rows {
        ...
        canvas.fill_rect(RectF::new(x, y, fw, 1.0), 128);
    }
}
