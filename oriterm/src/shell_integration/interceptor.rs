...
impl<T: EventListener> RawInterceptor<'_, T> {
    ...
    fn handle_osc7(&mut self, params: &[&[u8]]) {
        ...
        // URI format: `file://hostname/path` or `file:///path` (empty hostname).
        let path = parse_osc7_path(uri);
        if !path.is_empty() {
            let owned = path.to_owned();
            *self.term.cwd_mut() = Some(owned.clone());
            self.term.set_has_explicit_title(false);
            self.term.mark_title_dirty();
            self.term
                .event_listener()
                .send_event(oriterm_core::Event::Cwd(owned));
        }
    }
...
pub(super) fn parse_osc7_path(uri: &str) -> &str {
    ...
    // `memchr`-style linear scan for the `/` byte.
    after_scheme
        .as_bytes()
        .iter()
        .position(|&b| b == b'/')
        .map_or(after_scheme, |pos| {
            // SAFETY: `pos` is a byte offset at a `/` boundary, which is
            // always valid UTF-8.
            after_scheme.get(pos..).unwrap_or(after_scheme)
        })
}
