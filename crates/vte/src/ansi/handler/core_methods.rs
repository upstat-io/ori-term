//! Core / upstream `Handler` trait method declarations.
//!
//! Defines `handler_core_methods!()` — a `macro_rules!` that expands to
//! the method declarations for the upstream-compatible portion of the
//! `Handler` trait (cursor, modes, charsets, sixel/DCS/APC/iTerm2-file
//! entry points).
//!
//! `handler/mod.rs` invokes this macro inside the `pub trait Handler {... }`
//! body alongside the vendored-OSC and DEC-private method macros.

macro_rules! handler_core_methods {
    () => {
 ...
 ///
 /// `ps` selects the presentation state format (Ps=1 → DECCIR cursor
 /// information report; Ps=2 → DECTABSR tab-stop report).
 /// `pt` contains the payload bytes between the `$ t` introducer and the
 /// ST terminator. Bytes are passed as `&[u8]` — the VTE parser emits raw
 /// bytes before any UTF-8 validation and consumers must not assume the
 /// payload is a valid UTF-8 string.
        fn decrsps(&mut self, _ps: u16, _pt: &[u8]) {}
...
pub(crate) use handler_core_methods;
