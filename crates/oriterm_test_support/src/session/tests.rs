...
use super::{
    PtySession, TACK_PINNED_MAJOR, TACK_PINNED_MINOR, check_tack_version_with_emit,
    infocmp_available, parse_tack_version, tack_available, tack_runner_available_combine,
    tic_available, tool_available, unsupported_tack_diagnostic, vttest_available,
};
...
// ============================================================
// 05.0.c Tack version gate — unit tests.
//
// All tests in this section call the PURE helpers
// (`parse_tack_version`, `unsupported_tack_diagnostic`,
// `check_tack_version_with_emit`, `tack_runner_available_combine`)
// rather than spawning real tack. The pure-helper split lets the
// matrix run on hosts without tack installed AND in
// `cargo test --release`. The actual `tack_version_supported()`
// public function only adds the `tack -V` shell-out + the
// `eprintln!` emit closure — both verified by the pure tests
// below.
// ============================================================
