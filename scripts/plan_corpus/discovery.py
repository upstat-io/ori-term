...
_CONTAINER_DIRS = frozenset({"plans", "completed", "bug-tracker"})


def _classify_directory(dirpath: Path) -> DirClass:
    """Stage 1: classify a directory under plans/ or bug-tracker/."""
    dirpath = dirpath.resolve()
    if dirpath.is_relative_to(PLANS_DIR):
        rel = dirpath.relative_to(PLANS_DIR)
    elif dirpath.is_relative_to(BUG_TRACKER_DIR):
        # Preserve the 'bug-tracker' segment so CONTAINER detection below
        # matches when rel.parts[-1] == "bug-tracker" at depth 0.
        rel_from_bt = dirpath.relative_to(BUG_TRACKER_DIR)
        rel = Path("bug-tracker") / rel_from_bt if rel_from_bt.parts else Path("bug-tracker")
    else:
        rel = dirpath
    ...
    # bug-tracker/ at the root is a CONTAINER; its internal sections are
    # defect-tracking indexes (flat files, no per-bug subdirectories today).
    # bug-tracker/plans/ is the Fallback R carve-out for unowned-crate bugs;
    # each BUG-XX-NNN/ subdir is a PLAN_CANDIDATE (will have index.md).
    if dirpath == BUG_TRACKER_DIR:
        return DirClass.CONTAINER
...
class Corpus:
    ...
    audit_sections: dict[Path, dict] = field(default_factory=dict)
...
def discover_corpus(
    ...
    Args:
        root: Plans root directory. Defaults to PLANS_DIR. When the default
              is used, BUG_TRACKER_DIR is also walked (since bug-tracker is
              a wrapper-root sibling of plans/, not a subdirectory). A
              caller-supplied non-default root is walked alone — tests that
              pass a synthetic tmp_path get that path only.
    ...
    walk_roots: list[Path] = [root]
    if use_default_root and BUG_TRACKER_DIR.is_dir():
        walk_roots.append(BUG_TRACKER_DIR)

    # Stage 1 + 2: walk directories
    for walk_root in walk_roots:
        for dirpath_str, dirnames, filenames in os.walk(walk_root):
...
def _classify_and_store(path: Path, corpus: Corpus) -> None:
    ...
    bucket = {
        ...
        FileClass.AUDIT_SECTION: corpus.audit_sections,
    }.get(fc)
    ...
    if fc is None:
        ...
        return LoadResult(err=Finding(
            ...
            recommended_fix=(
                ...
                "plans/roadmap/section-*.md, bug-tracker/section-*.md, "
                "bug-tracker/fix-BUG-*.md, or plans/completed/*/index.md)"
            ),
        ))
