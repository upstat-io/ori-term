...
in `bug-tracker/section-*.md` files. It defines:

  - Lifecycle marker regex constants (Superseded, Blocked, blocked-by)
...
First match wins. The order encodes "what does this bug need next":
  ...
  2. **Blocked: / **Blocked**:** — bug is waiting on a dependency
     (different from Superseded — no plan owns the fix yet).
  3. **<!-- blocked-by:** — cross-section blocker tag; the blocking work
     lives elsewhere in the corpus.

## Deprecated markers

  - **Escalated to plan: / Escalated:** — retired under the single-route
    invariant (every bug becomes `bug-tracker/plans/BUG-XX-NNN/` via
    `/fix-bug` Phase 1.6 scaffolding). The `BUG_ESCALATED_RE` constant
    is retained so migration tooling can detect fossil markers in legacy
    tracker entries; it is NOT in the exclusion precedence chain. Convert
    fossils to `Superseded by:` (when a target plan exists) or strip the
    line (informational scope notes preserved as plain text in the body).
...
# Deprecated under single-route — retained for fossil detection in migration
# tooling; NOT in the exclusion precedence chain. See module docstring.
...
# Precedence: Superseded > Blocked > blocked-by.
# Each entry is (regex, exclusion_reason). First match wins.
# `BUG_ESCALATED_RE` is intentionally absent — see module docstring §Deprecated.
...
def classify_bug_exclusion(body_text: str) -> str | None:
    ...
    `Escalated:` / `Escalated to plan:` is NOT classified as an exclusion
    under the single-route invariant; bugs carrying that fossil marker are
    actionable by `/fix-bug` (Phase 1.6 scaffolds the plan dir).
    """
...
def parse_bug_entries(text: str, source_file: str) -> Iterator[BugEntry]:
    ...
    layout in `bug-tracker/section-*.md` files.
