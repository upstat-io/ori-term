...
# bug-tracker is a wrapper-root sibling of plans/, not a subdirectory of it.
# Defect-tracking is an INDEX subsystem with its own lifecycle (open/closed
# bugs + peer resolution-plans), not a plan under plans/.
BUG_TRACKER_DIR = REPO_ROOT / "bug-tracker"
...
class FindingSubtype(enum.Enum):
    ...
    # PLAN_COHERENCE subtypes
    # Surface drift between cross-artifact plan claims (overview / index /
    # mission-success / dependency-graph) and the executable section files
    # they describe. Distinct from DEAD_REFERENCE.SECTION_FILE_NOT_FOUND
    # (which is YAML-derived from `depends_on:` etc.) — these subtypes
    # cover BODY-prose references that the schema validator does not see.
    MISSING_REFERENCED_SECTION = "missing_referenced_section"
    FRONTMATTER_SECTION_FILENAME_MISMATCH = "frontmatter_section_filename_mismatch"
    OVERVIEW_LABEL_DRIFT = "overview_label_drift"
    MS_CRITERION_NO_SECTION = "ms_criterion_no_section"
    DEPGRAPH_DEAD_EDGE = "depgraph_dead_edge"
...
_CATEGORY_SUBTYPES: dict[FindingCategory, frozenset[FindingSubtype]] = {
    ...
    FindingCategory.PLAN_COHERENCE: frozenset({
        FindingSubtype.MISSING_REFERENCED_SECTION,
        FindingSubtype.FRONTMATTER_SECTION_FILENAME_MISMATCH,
        FindingSubtype.OVERVIEW_LABEL_DRIFT,
        FindingSubtype.MS_CRITERION_NO_SECTION,
        FindingSubtype.DEPGRAPH_DEAD_EDGE,
    }),
}
