...
A plan that lists `supersedes: ["bug-tracker/fix-BUG-XX-NNN.md"]` in
...
The `Superseded by:` lifecycle marker has two endpoints that must agree:
  ...
  * **Bug entry side**: `bug-tracker/section-NN-*.md` body line
    `Superseded by: plans/{plan}/`.
  * **Plan side**: `plans/{plan}/00-overview.md` frontmatter
    `supersedes: ["bug-tracker/fix-BUG-XX-NNN.md"]`.
...
class MissingSupersedeMarker:
    ...
    path to this bug's fix-section file (`bug-tracker/fix-BUG-XX-NNN.md`),
    ...
    def bug_section_path(self) -> str:
        return f"bug-tracker/{self.bug_section_file}"
...
_FIX_BUG_REF_RE = re.compile(
    r"bug-tracker/fix-(BUG-\d{2}-\d{3})\.md",
)
...
def parse_plan_supersedes(overview_path: Path) -> list[str]:
    ...
    Returns the raw string entries (e.g., `"bug-tracker/fix-BUG-04-074.md"`).
...
def find_supersede_drift(
    ...
    Bug-tracker dir defaults to the wrapper-root sibling `BUG_TRACKER_DIR`
    (post-redesign: `bug-tracker/` lives next to `plans/`, not under it).
    """
    if bug_tracker_dir is None:
        from .types import BUG_TRACKER_DIR
        bug_tracker_dir = BUG_TRACKER_DIR

    # Build map: BUG-XX-NNN -> list of plans claiming to supersede it.
    # Walk BOTH plans_dir (regular peer plans + roadmap) AND bug_tracker_dir/plans
    # (Fallback R resolution-plans that supersede bugs with no roadmap owner).
    claims: dict[str, list[tuple[str, str]]] = {}  # bug_id -> [(plan_name, overview_path)]
    fallback_r_root = bug_tracker_dir / "plans"
    overview_roots = [plans_dir]
    if fallback_r_root.is_dir():
        overview_roots.append(fallback_r_root)
    seen: set[Path] = set()
    for root in overview_roots:
        for overview_path in root.rglob("00-overview.md"):
            if overview_path in seen:
                continue
            seen.add(overview_path)
...
class PlannedEdit:
    ...
    """
    file_path: str          # e.g. "bug-tracker/section-04-codegen-llvm.md"
    bug_id: str
    ...
    for finding in report.missing_markers:
        edits.append(PlannedEdit(
            ...
            rationale=(
                ...
                f"`supersedes: [\"bug-tracker/fix-{finding.bug_id}.md\"]` "
