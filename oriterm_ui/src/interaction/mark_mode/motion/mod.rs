...
pub fn move_left(c: AbsCursor, b: GridBounds) -> AbsCursor {
    if c.col > 0 {
        AbsCursor {
            col: c.col - 1,
            ..c
...
pub fn move_right(c: AbsCursor, b: GridBounds) -> AbsCursor {
    if c.col + 1 < b.cols {
        AbsCursor {
            col: c.col + 1,
            ..c
...
pub fn move_up(c: AbsCursor) -> AbsCursor {
    AbsCursor {
...
pub fn move_down(c: AbsCursor, b: GridBounds) -> AbsCursor {
...
pub fn page_up(c: AbsCursor, b: GridBounds) -> AbsCursor {
    AbsCursor {
...
pub fn page_down(c: AbsCursor, b: GridBounds) -> AbsCursor {
    let last = b.total_rows.saturating_sub(1);
    AbsCursor {
...
pub fn line_start(c: AbsCursor) -> AbsCursor {
    AbsCursor { col: 0, ..c }
}

/// Move cursor to the end of the current line.
pub fn line_end(c: AbsCursor, b: GridBounds) -> AbsCursor {
    AbsCursor {
        col: b.cols.saturating_sub(1),
        ..c
    }
...
pub fn buffer_start() -> AbsCursor {
    AbsCursor { abs_row: 0, col: 0 }
}

/// Move cursor to the end of the buffer.
pub fn buffer_end(b: GridBounds) -> AbsCursor {
    AbsCursor {
...
pub fn word_right(c: AbsCursor, ctx: &WordContext, b: GridBounds) -> AbsCursor {
    if c.col < ctx.we {
        AbsCursor {
            abs_row: c.abs_row,
            col: ctx.we,
